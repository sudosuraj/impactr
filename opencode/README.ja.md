...
# パッケージマネージャー
npm i -g impactr-ai@latest        # bun/pnpm/yarn でもOK
scoop install impactr             # Windows
choco install impactr             # Windows
brew install anomalyco/tap/impactr # macOS と Linux（推奨。常に最新）
brew install impactr              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S impactr            # Arch Linux (Stable)
paru -S impactr-bin               # Arch Linux (Latest from AUR)
mise use -g impactr               # どのOSでも
nix run nixpkgs#impactr           # または github:anomalyco/impactr で最新 dev ブランチ
```
...
Impactr はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/impactr/releases) から直接ダウンロードするか、[impactr.dev/download](https://impactr.dev/download) を利用してください。
...
1. `$IMPACTR_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.impactr/bin` - デフォルトのフォールバック
...
Impactr には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://impactr.dev/docs/agents) の詳細はこちら。

### ドキュメント

Impactr の設定については [**ドキュメント**](https://impactr.dev/docs) を参照してください。

### コントリビュート

Impactr に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### Impactr の上に構築する

Impactr に関連するプロジェクトで、名前に "impactr"（例: "impactr-dashboard" や "impactr-mobile"）を含める場合は、そのプロジェクトが Impactr チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/impactr) | [X.com](https://x.com/impactr)

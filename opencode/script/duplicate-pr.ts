...
import { createImpactr } from "@impactr-ai/sdk"
...
`)
  ...
  const impactr = await createImpactr({ port: 0 })
  ...
  try {
    ...
    const session = await impactr.client.session.create()
    const result = await impactr.client.session
      .prompt({

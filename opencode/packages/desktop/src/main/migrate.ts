...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.impactr.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `impactr.settings.dat` is special: it maps to the `impactr.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"impactr.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // impactr.settings.dat → the electron settings store ("impactr.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "impactr.global.dat").
  const storeName = filename === "impactr.settings.dat" ? "impactr.settings" : filename

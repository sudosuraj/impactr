...
import type { UpdaterState } from "@impactr-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@impactr-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.IMPACTR_CHANNEL === "prod" &&
...
const deepLinkEvent = "impactr:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__IMPACTR__ ??= {}
  const pending = window.__IMPACTR__.deepLinks ?? []
  window.__IMPACTR__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `impactr.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("impactr.global.dat").getItem("language")

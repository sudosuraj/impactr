...
import { PromptInput } from "@impactr-ai/schema/prompt-input"
import { Session } from "@impactr-ai/schema/session"
import { Project } from "@impactr-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@impactr-ai/schema/schema"
...
import { SessionEvent } from "@impactr-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Impactr process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Impactr process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(

...
import { createImpactr, type ToolPart } from "@impactr-ai/sdk"
...
console.log("🚀 Starting impactr server...")
const impactr = await createImpactr({
  port: 0,
})
console.log("✅ Impactr server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await impactr.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new impactr session...")
    const { client, server } = impactr
    ...
    console.log("✅ Created impactr session:", createResult.data.id)
  ...
  console.log("📝 Sending to impactr:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Impactr response:", JSON.stringify(result, null, 2))

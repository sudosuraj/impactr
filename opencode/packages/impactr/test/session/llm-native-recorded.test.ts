...
import { ModelsDev } from "@impactr-ai/core/models-dev"
...
import { LLMEvent, LLMResponse } from "@impactr-ai/llm"
import { RequestExecutor } from "@impactr-ai/llm/route"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.IMPACTR_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["impactr", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("IMPACTR_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("IMPACTR_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["impactr", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["impactr", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.IMPACTR_RECORD_CONSOLE_TOKEN && process.env.IMPACTR_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.impactr,
        name: "Impactr Zen",
        env: ["IMPACTR_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.IMPACTR_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.IMPACTR_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.IMPACTR_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["impactr", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("IMPACTR_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("IMPACTR_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("IMPACTR_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set IMPACTR_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the impactr LLM stack remain real.
  const metadata = {

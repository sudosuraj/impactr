...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "impactr.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "impactr.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "impactr.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "impactr.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["impactr.json", "impactr.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "impactr.json"),
        path.join(project, "impactr.jsonc"),
      ])
    })
  })

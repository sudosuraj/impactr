...
import type { Event, ImpactrClient } from "@impactr-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<ImpactrClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "impactr-acp-permission-"))

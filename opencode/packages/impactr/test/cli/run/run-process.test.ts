// Subprocess integration tests for `impactr run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `impactr.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `IMPACTR_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("impactr run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, impactr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* impactr.run("say hi")
        impactr.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, impactr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* impactr.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, impactr }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* impactr.run("think", { extraArgs: ["--thinking"] })
        impactr.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* impactr.run("think again")
        impactr.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, impactr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* impactr.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, impactr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* impactr.run("say hi", { format: "json" })
        impactr.expectExit(result, 0)

        const events = impactr.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ impactr }) =>
      Effect.gen(function* () {
        const result = yield* impactr.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, impactr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* impactr.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, impactr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* impactr.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, impactr }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* impactr.run("request permission", { permission: { bash: "ask" } })
        impactr.expectExit(denied, 0)
        ...
        const allowed = yield* impactr.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        impactr.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* impactr.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        impactr.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, impactr }) =>
      Effect.gen(function* () {
        ...
        const server = yield* impactr.serve()

        const result = yield* impactr.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, impactr }) =>
      Effect.gen(function* () {
        const result = yield* impactr.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, impactr }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* impactr.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()

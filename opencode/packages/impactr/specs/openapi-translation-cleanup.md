...
Trim `packages/impactr/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/impactr/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/impactr`.
- `[x]` `bun typecheck` from `packages/impactr`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/impactr/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/impactr`.
- `[x]` `bun dev generate > /tmp/impactr-openapi.json` from `packages/impactr`.
...
- Edit `packages/impactr/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/impactr-openapi.json` from `packages/impactr`.
...
- Check source schemas in files like `packages/impactr/src/session/schema.ts`, `packages/impactr/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/impactr/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/impactr/src/server/routes/instance/httpapi/errors.ts` or add new ones there.

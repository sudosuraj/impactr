...
Practical reference for a future `packages/server` split after the impactr
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/impactr`.
...
Until enough shared service code lives outside `packages/impactr`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/impactr`

It should not import `packages/impactr` services while `packages/impactr`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on impactr-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/impactr` implementation details.

...
Delete `packages/impactr/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/impactr/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/impactr/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/impactr`
- TUI plugin loader tests in `packages/impactr` if runtime plugin API wiring changed

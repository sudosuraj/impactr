...
- Author package entrypoint is `@impactr-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/impactr-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@impactr-ai/plugin/tui`.
- `@impactr-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@impactr-ai/plugin/tui"
...
- The value is a semver range checked against the running Impactr version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.impactr` is absent, no check is performed (backward compatible).
...
- `impactr plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `impactr plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.impactr` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.impactr`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.impactr`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.impactr` dir or `IMPACTR_CONFIG_DIR`, Impactr installs `@impactr-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@impactr-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Impactr's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Impactr registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@impactr-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"impactr"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `impactr.default`.
...
- Local plugins persist installed themes under the local `.impactr/themes` area near the plugin config source.
...
- `--pure` / `IMPACTR_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.impactr/plugins/tui-smoke.tsx`
- Local vim plugin: `.impactr/plugins/tui-vim.tsx`
- Local smoke config: `.impactr/tui.json`
- Local smoke theme: `.impactr/plugins/smoke-theme.json`

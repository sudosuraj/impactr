...
export const DEPRECATED_PLUGIN_PACKAGES = ["impactr-openai-codex-auth", "impactr-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, impactrVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(impactrVersion) || semver.major(impactrVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.impactr
  if (typeof range !== "string") return
  if (!semver.satisfies(impactrVersion, range)) {
    throw new Error(`Plugin requires impactr ${range} but running ${impactrVersion}`)
  }
}

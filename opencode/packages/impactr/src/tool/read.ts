...
import { NonNegativeInt } from "@impactr-ai/core/schema"
...
export const Parameters = Schema.Struct({
  ...
  tail: Schema.optional(Schema.Boolean).annotate({
    description: "If true, reads from the end of the file instead of the beginning",
  }),
})
...
>(
  ...
  Effect.gen(function* () {
    ...
    const lines = Effect.fn("ReadTool.lines")(function* (filepath: string, opts: { limit: number; offset: number; tail?: boolean }) {
      ...
      if (opts.tail) {
        ...
        for (let i = raw.length - 1; i >= 0; i--) {
          const lineSize = Buffer.byteLength(raw[i], "utf-8") + 1
          if (size + lineSize <= MAX_BYTES) {
            kept.unshift(raw[i])
            size += lineSize
          } else {
            flags.cut = true
        ...
        return { raw: kept, count: flags.count, cut: flags.cut, more: flags.count > kept.length, offset: Math.max(1, flags.count - kept.length + 1) }
      }
    ...
    const run = Effect.fn("ReadTool.execute")(function* (
      ...
      const file = yield* lines(filepath, { limit: params.limit ?? DEFAULT_READ_LIMIT, offset: params.offset || 1, tail: params.tail })

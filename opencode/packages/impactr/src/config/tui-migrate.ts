...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from impactr.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const impactr = await impactrFiles(input)
  for (const file of impactr) {
...
async function impactrFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "impactr"),
    ...(await Filesystem.findUp(["impactr.json", "impactr.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "impactr"))
  }
  if (Flag.IMPACTR_CONFIG) files.push(Flag.IMPACTR_CONFIG)

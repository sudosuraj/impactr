...
import { ConfigMCPV1 } from "@impactr-ai/core/v1/config/mcp"
...
import { McpEvent } from "@impactr-ai/schema/mcp-event"

const DEFAULT_TIMEOUT = 30_000
const CLIENT_OPTIONS = {
  capabilities: {
    // https://github.com/anomalyco/impactr/issues/11948
    // sampling: {},
    // https://github.com/anomalyco/impactr/issues/23066
    // elicitation: {},
    // https://github.com/anomalyco/impactr/issues/2308
    roots: {},
    // https://github.com/anomalyco/impactr/issues/28567
    // tasks: {},
  },
...
function createClient(directory: string) {
  const client = new Client({ name: "impactr", version: InstallationVersion }, CLIENT_OPTIONS)
...
export interface Interface {
  ...
  readonly tools: (clientNames?: string[]) => Effect.Effect<Record<string, Tool>>
...
export class Service extends Context.Service<Service, Interface>()("@impactr/MCP") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: impactr mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
    ...
    const tools = Effect.fn("MCP.tools")(function* (clientNames?: string[]) {
      ...
      for (const [clientName, client] of Object.entries(s.clients)) {
        if (clientNames && !clientNames.includes(clientName)) continue

...
Du kan legge til eksterne verktøy i Impactr ved å bruke _Model Context Protocol_ eller MCP. Impactr støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Impactr Config](https://impactr.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="impactr.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/impactr`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Impactr håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Impactr:
...
Hvis serveren krever autentisering, vil Impactr be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `impactr mcp auth <server-name>`.
...
```bash
impactr mcp auth my-oauth-server
```
...
```bash
impactr mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Impactr lagre tokens sikkert i `~/.local/share/impactr/mcp-auth.json`.
...
impactr mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Impactr, sammen med innebygde verktøy. Så du kan administrere dem gjennom Impactr-konfigurasjonen som alle andre verktøy.
...
```json title="impactr.json" {11, 14-18}
{
  "$schema": "https://impactr.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
impactr mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Impactr til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Impactr-samtale oppført som en separat økt.

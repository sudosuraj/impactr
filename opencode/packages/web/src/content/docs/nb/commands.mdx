...
Opprett `.impactr/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom Impactr-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i Impactr [config](/docs/config):
...
Impactr inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).

...
description: Skriv dine egne programtillegg for å utvide Impactr.
---

Plugins lar deg utvide Impactr ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Impactr.
...
- `.impactr/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/impactr/node_modules/`.
...
1. Global konfigurasjon (`~/.config/impactr/impactr.json`)
2. Prosjektkonfigurasjon (`impactr.json`)
3. Global plugin-katalog (`~/.config/impactr/plugins/`)
4. Prosjektpluginkatalog (`.impactr/plugins/`)
...
Impactr kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Impactr SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Impactr.
...
:::note
Hvis du bruker Impactr-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Impactr fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Impactr:
...
`tool`-hjelperen lager et tilpasset verktøy som Impactr kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Impactr sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://impactr.dev/docs/sdk) for detaljer.

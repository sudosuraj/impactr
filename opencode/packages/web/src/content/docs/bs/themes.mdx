...
U Impactr mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Impactr koristi nasu `impactr` temu.
...
Impactr dolazi sa vise ugradenih tema.
...
- Zele da Impactr odgovara izgledu njihovog terminala
...
Impactr podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/impactr/themes/*.json` ili `$XDG_CONFIG_HOME/impactr/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.impactr/themes/*.json`
4. **Trenutni radni direktorij** - `./.impactr/themes/*.json`

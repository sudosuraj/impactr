...
Da biste otklonili probleme s Impactr, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\impactr\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `impactr --log-level DEBUG`.

---

## Pohrana

Impactr pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/impactr/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\impactr`
...
Impactr Desktop pokreće lokalni Impactr server (`impactr-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Impactr` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/impactr/impactr.jsonc` (ili `~/.config/impactr/impactr.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/impactr/impactr.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\impactr\impactr.jsonc`
...
Impactr također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/impactr/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\impactr\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Impactr mogao ponovo izgraditi.

1. Potpuno zatvorite Impactr Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/impactr`
- **Linux**: obrišite `~/.cache/impactr` (ili pokrenite `rm -rf ~/.cache/impactr`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\impactr`

3. Ponovo pokrenite Impactr Desktop.

---

### Rješavanje problema sa vezom na serveru

Impactr Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `impactr.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `IMPACTR_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `IMPACTR_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Impactr Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Impactr karakteristikama.

---

### Obavještenja se ne prikazuju

Impactr Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Impactr u postavkama vašeg OS-a, i
...
1. Zatvorite Impactr Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Impactr Desktop aplikacije):

- `impactr.settings.dat` (URL zadanog servera za desktop)
- `impactr.global.dat` i `impactr.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `impactr models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\impactr`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Impactr dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\impactr`

2. Ponovo pokrenite Impactr da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Impactr da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Impactr će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.

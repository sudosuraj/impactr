---
title: Server
description: Komunicirajte s impactr serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `impactr serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji impactr klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `impactr.local` |
...
Postavite `IMPACTR_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `impactr`, ili postavite `IMPACTR_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `impactr serve` i za `impactr web`.
...
Kada pokrenete `impactr`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite impactr server za programsku interakciju sa impactr-om.
:::

Ova arhitektura omogucava impactr podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `impactr serve` da startate standalone server. Ako je impactr TUI vec pokrenut, `impactr serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Impactr [IDE](/docs/ide) pluginovi.
...
impactr server izlaže sljedece API-je.

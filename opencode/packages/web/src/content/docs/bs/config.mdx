---
title: Konfiguracija
description: Korištenje Impactr JSON konfiguracije.
---

Možete konfigurirati Impactr koristeći JSON konfiguracijski fajl.

---

## Format

Impactr podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/impactr`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/impactr/impactr.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`IMPACTR_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`impactr.json` u projektu) - postavke specifične za projekat
5. **`.impactr` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`IMPACTR_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.impactr` i `~/.config/impactr` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/impactr` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Impactr konfiguraciju u `~/.config/impactr/impactr.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/impactr/tui.json`.
...
Dodajte `impactr.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Impactr pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `IMPACTR_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `IMPACTR_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.impactr` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.impactr`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`impactr.dev/config.json`**](https://impactr.dev/config.json).

TUI konfiguracija koristi [**`impactr.dev/tui.json`**](https://impactr.dev/tui.json).
...
Koristite `IMPACTR_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `impactr.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `impactr serve` i `impactr web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Impactr server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `impactr.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Impactr konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Impactr pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/impactr/agents/` ili `.impactr/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Impactr će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`impactr run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/impactr/commands/` ili `.impactr/commands/`. [Saznajte više ovdje](/docs/commands).
...
Impactr će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Impactr **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Impactr sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.impactr/plugins/` ili `~/.config/impactr/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Impactr da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.

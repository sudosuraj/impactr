...
Možete dodati vanjske alate u Impactr koristeći _Model Context Protocol_, ili MCP. Impactr podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Impactr Config](https://impactr.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/impactr`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Impactr automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Impactr će:
...
Ako server zahtijeva autentifikaciju, Impactr će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `impactr mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Impactr sigurno čuva tokene u `~/.local/share/impactr/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Impactr, zajedno s ugrađenim alatima. Možete njima upravljati kroz Impactr konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Impactr sa vašim Sentry nalogom.

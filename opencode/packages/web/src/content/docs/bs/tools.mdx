...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Impactr dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Impactr.
...
:::note
Ovaj alat je dostupan samo kada je `IMPACTR_EXPERIMENTAL_LSP_TOOL=true` (ili `IMPACTR_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Impactr provajdera ili kada je varijabla `IMPACTR_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Impactr:

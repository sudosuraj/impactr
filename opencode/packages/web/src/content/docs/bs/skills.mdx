...
Agent skills omogucavaju Impactr da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Impactr pretrazuje ove lokacije:

- Konfiguracija projekta: `.impactr/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/impactr/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Impactr ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.impactr/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/impactr/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.impactr/skills/git-release/SKILL.md` ovako:
...
Impactr navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `impactr.json`:
...
**Za ugradene agente** (u `impactr.json`):

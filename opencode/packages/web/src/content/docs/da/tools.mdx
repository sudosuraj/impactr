...
Her er alle de innebygde verktøyene som er tilgængelige i Impactr.
...
:::note
Dette verktøyet er bare tilgængelig når `IMPACTR_EXPERIMENTAL_LSP_TOOL=true` (eller `IMPACTR_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Impactr-leverandøren eller når miljøvariabelen `IMPACTR_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).

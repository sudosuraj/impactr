---
title: Server
description: Interager med impactr-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `impactr serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en impactr-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `impactr.local` |
...
Indstil `IMPACTR_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `impactr`, eller indstil `IMPACTR_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `impactr serve` og `impactr web`.
...
Når du kører `impactr` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug impactr-serveren til at interagere med impactr programmatisk.
:::

Denne arkitektur lader impactr understøtte flere klienter og lader dig interagere med impactr programmatisk.

Du kan køre `impactr serve` for at starte en selvstændig server. Hvis du har
impactr TUI kørende, vil `impactr serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Impactr [IDE](/docs/ide) plugins.
...
Impactr-serveren viser følgende API'er.

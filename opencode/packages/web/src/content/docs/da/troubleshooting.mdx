...
For at fejlfinde problemer med Impactr, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\impactr\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `impactr --log-level DEBUG`.

---

## Lagring

impactr gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/impactr/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\impactr`
...
Impactr Desktop kører en lokal Impactr-server (`impactr-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Impactr`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/impactr/impactr.jsonc` (eller `~/.config/impactr/impactr.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/impactr/impactr.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\impactr\impactr.jsonc`
...
Impactr kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/impactr/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\impactr\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Impactr kan genopbygge den.

1. Afslut Impactr Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/impactr`
- **Linux**: slet `~/.cache/impactr` (eller kør `rm -rf ~/.cache/impactr`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\impactr`

3. Start Impactr Desktop på ny.

---

### Løs problemer med serverforbindelse

Impactr Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `impactr.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `IMPACTR_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `IMPACTR_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Impactr Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Impactr.

---

### Meddelelser vises ikke

Impactr Desktop viser kun systemvarsler når:

- varsler er aktiveret for Impactr i dine OS-indstillinger, og
...
1. Afslut Impactr Desktop.
2. Find og slet disse filer (de findes i Impactr Desktop-appens datamappe):

- `impactr.settings.dat` (skrivebordsstandardserver URL)
- `impactr.global.dat` og `impactr.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Impactr:
...
For at finde ud af hvilke modeller du har adgang til, kør `impactr models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\impactr`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. impactr installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\impactr`

2. Start impactr på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge impactr til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
impactr vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.

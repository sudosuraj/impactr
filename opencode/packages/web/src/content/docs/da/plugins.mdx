---
title: Plugins
description: Skriv dine egne plugins for at udvide Impactr.
---

Plugins giver dig mulighed for at udvide Impactr ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Impactrs standardadfærd.
...
- `.impactr/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/impactr/node_modules/`.
...
1. Global konfiguration (`~/.config/impactr/impactr.json`)
2. Projektkonfiguration (`impactr.json`)
3. Global plugin-mappe (`~/.config/impactr/plugins/`)
4. Projekt plugin bibliotek (`.impactr/plugins/`)
...
Impactr kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En impactr SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide impactr.
...
:::note
Hvis du bruger Impactr desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå impactr i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til impactr:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som impactr kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for impactr sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://impactr.dev/docs/sdk) for detaljer.

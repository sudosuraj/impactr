...
Agent Skills erlauben Impactr, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Impactr sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Impactr vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.impactr/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/impactr/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.impactr/skills/git-release/SKILL.md` zum Beispiel so:
...
Impactr listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `impactr.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `impactr.json`):

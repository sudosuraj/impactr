...
Mit Impactr kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Impactr das `impactr`-Theme.
...
Impactr liefert mehrere eingebaute Themes mit.
...
- Impactr optisch ans Terminal anpassen wollen
...
Impactr unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/impactr/themes/*.json` oder `$XDG_CONFIG_HOME/impactr/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.impactr/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.impactr/themes/*.json`

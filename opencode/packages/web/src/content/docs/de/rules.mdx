---
title: Regeln
description: Lege eigene Anweisungen fuer impactr fest.
---

Du kannst impactr ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in impactr den Befehl `/init` aus.
...
So kann impactr sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht impactr sie zu erweitern.
...
impactr kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/impactr/AGENTS.md` speichern.
Diese gelten in allen impactr-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Impactr dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/impactr/AGENTS.md` existiert)
...
Beim Start sucht impactr in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/impactr/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `impactr.json` oder in `~/.config/impactr/impactr.json` angeben.
...
impactr parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit impactr.json

Empfohlen ist das `instructions`-Feld in der `impactr.json`:
...
Du kannst impactr explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `impactr.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::

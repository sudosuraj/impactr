---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Impactr zu erweitern.
---

Mit Plugins können Sie Impactr erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Impactr zu ändern.
...
- `.impactr/plugins/` – Plugins auf Projektebene
- `~/.config/impactr/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/impactr/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/impactr/impactr.json`)
2. Projektkonfiguration (`impactr.json`)
3. Globales Plugin-Verzeichnis (`~/.config/impactr/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.impactr/plugins/`)
...
Impactr führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Impactr-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Impactr erweitern können.
...
:::note
Wenn Sie die Impactr-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Impactr `.env`-Dateien liest:
...
Plugins können Impactr auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Impactr aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Impactr zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://impactr.dev/docs/sdk).

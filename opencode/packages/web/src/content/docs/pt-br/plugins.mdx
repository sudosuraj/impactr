---
title: Plugins
description: Escreva seus próprios plugins para estender o impactr.
---

Plugins permitem que você estenda o impactr conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do impactr.
...
- `.impactr/plugins/` - Plugins em nível de projeto
- `~/.config/impactr/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/impactr/node_modules/`.
...
1. Configuração global (`~/.config/impactr/impactr.json`)
2. Configuração do projeto (`impactr.json`)
3. Diretório de plugins global (`~/.config/impactr/plugins/`)
4. Diretório de plugins do projeto (`.impactr/plugins/`)
...
O impactr executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do impactr para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o impactr.
...
:::note
Se você estiver usando o aplicativo desktop impactr, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o impactr de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao impactr:
...
O helper `tool` cria uma ferramenta personalizada que o impactr pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o impactr junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://impactr.dev/docs/sdk) para detalhes.

---
title: Servidor
description: Interaja com o servidor impactr via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `impactr serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente impactr pode usar.
...
```bash
impactr serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `impactr.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
impactr serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `IMPACTR_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `impactr`, ou defina `IMPACTR_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `impactr serve` quanto ao `impactr web`.

```bash
IMPACTR_SERVER_PASSWORD=your-password impactr serve
```
...
Quando você executa `impactr`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor impactr para interagir com o impactr programaticamente.
:::

Essa arquitetura permite que o impactr suporte múltiplos clientes e permite que você interaja com o impactr programaticamente.

Você pode executar `impactr serve` para iniciar um servidor autônomo. Se você tiver o TUI do impactr em execução, `impactr serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do impactr [IDE](/docs/ide).
...
O servidor impactr expõe as seguintes APIs.

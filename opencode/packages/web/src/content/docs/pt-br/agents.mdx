...
Existem dois tipos de agentes no impactr; agentes primários e subagentes.
...
impactr vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Impactr vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Impactr vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Impactr, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `impactr.json`:
...
- Global: `~/.config/impactr/agents/`
- Por projeto: `.impactr/agents/`

```markdown title="~/.config/impactr/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o impactr usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do impactr quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do impactr usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Impactr Zen](/docs/zen), você usaria `impactr/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="impactr.json" {3-6,9-12}
{
  "$schema": "https://impactr.dev/config.json",
  "tools": {
    "write": true,
...
```json title="impactr.json" {3-5,8-10}
{
  "$schema": "https://impactr.dev/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `impactr models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/impactr).
:::
...
```markdown title="~/.config/impactr/agents/docs-writer.md"
---
...
```markdown title="~/.config/impactr/agents/security-auditor.md"
---

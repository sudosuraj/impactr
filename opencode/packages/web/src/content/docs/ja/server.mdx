---
title: サーバー
description: HTTP 経由で Impactr サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`impactr serve` コマンドは、Impactr クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `impactr.local` |
...
HTTP 基本認証でサーバーを保護するには、`IMPACTR_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `impactr` になるか、`IMPACTR_SERVER_USERNAME` を設定してオーバーライドします。これは、`impactr serve` と `impactr web` の両方に当てはまります。
...
`impactr` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Impactr サーバーを使用して、プログラムで Impactr と対話します。
:::

`impactr serve` を実行してスタンドアロンサーバーを起動できます。すでに Impactr TUI を実行している場合でも、`impactr serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Impactr [IDE](/docs/ide) プラグインによって使用されます。

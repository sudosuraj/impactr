...
أنشئ الملف `.impactr/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Impactr أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Impactr:
...
- على مستوى النظام: `~/.config/impactr/commands/`
- لكل مشروع: `.impactr/commands/`
...
يتضمن impactr عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).

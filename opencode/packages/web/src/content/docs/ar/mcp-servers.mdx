...
يمكنك إضافة أدوات خارجية إلى Impactr باستخدام _Model Context Protocol_، أو MCP. يدعم Impactr خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Impactr](https://impactr.dev/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/impactr` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Impactr تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Impactr بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Impactr المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `impactr mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Impactr الرموز بشكل آمن في `~/.local/share/impactr/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Impactr إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Impactr مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Impactr بحساب Sentry الخاص بك.

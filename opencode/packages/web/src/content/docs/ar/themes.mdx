...
مع Impactr يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Impactr سمتنا `impactr`.
...
يأتي Impactr مع عدة سمات مدمجة.
...
- يريدون أن يطابق Impactr مظهر terminal لديهم
...
يدعم Impactr نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/impactr/themes/*.json` أو `$XDG_CONFIG_HOME/impactr/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.impactr/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.impactr/themes/*.json`

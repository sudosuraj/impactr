...
Create `.impactr/commands/test.md`:
...
You can add custom commands through the Impactr config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Impactr [config](/docs/config):
...
impactr includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).

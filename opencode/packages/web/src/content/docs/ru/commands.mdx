...
Создайте `.impactr/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию impactr или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем impactr [config](/docs/config):
...
- Глобальный: `~/.config/impactr/commands/`
- Для каждого проекта: `.impactr/commands/`
...
impactr включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).

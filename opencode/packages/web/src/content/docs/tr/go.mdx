...
Impactr Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Impactr'daki diğer sağlayıcılar gibi çalışır. Impactr Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Impactr'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Impactr Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Impactr Go, Impactr'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Impactr Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Impactr Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Impactr Go'ya abone olabilir.
:::
...
Impactr Go aşağıdaki limitleri içerir:
...
Impactr yapılandırmanızdaki [model id](/docs/config/#models) formatı `impactr-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `impactr-go/kimi-k2.7-code` kullanmalısınız.
...
Impactr Go'yu şu amaçlarla oluşturduk:
...
4. Impactr ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.

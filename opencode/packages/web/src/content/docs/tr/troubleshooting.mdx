...
impactr ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\impactr\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `impactr --log-level DEBUG`.

---

## Depolama

impactr, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/impactr/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\impactr` yapistirin
...
impactr Desktop arka planda yerel bir impactr sunucusu (`impactr-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `impactr` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/impactr/impactr.jsonc` (veya `~/.config/impactr/impactr.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/impactr/impactr.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\impactr\impactr.jsonc` yapistirin
...
impactr diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/impactr/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\impactr\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), impactr'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. impactr Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/impactr`
- **Linux**: `~/.cache/impactr` dizinini silin (veya `rm -rf ~/.cache/impactr` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\impactr` yapistirin

3. impactr Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

impactr Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`impactr.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `IMPACTR_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `IMPACTR_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta impactr Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, impactr ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

impactr Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda impactr icin bildirimler etkinse
...
1. impactr Desktop'u kapatin
2. Su dosyalari bulun ve silin (impactr Desktop uygulama veri dizininde yer alirlar):

- `impactr.settings.dat` (desktop varsayilan sunucu URL'si)
- `impactr.global.dat` ve `impactr.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
impactr ile ilgili bir sorun yasiyorsaniz:
...
3. `impactr upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `impactr models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\impactr`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. impactr, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\impactr`

2. En guncel provider paketlerini yeniden kurmak icin impactr'u yeniden baslatin

Bu, impactr'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
impactr, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.

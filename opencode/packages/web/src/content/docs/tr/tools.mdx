...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. impactr yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
impactr içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `IMPACTR_EXPERIMENTAL_LSP_TOOL=true` (veya `IMPACTR_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca impactr provider kullanılırken veya `IMPACTR_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

impactr başlatırken etkinleştirmek için:

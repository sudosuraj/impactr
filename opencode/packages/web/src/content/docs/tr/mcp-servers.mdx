...
_Model Bağlam Protokolü_ veya MCP kullanarak impactr'a harici araçlar ekleyebilirsiniz. impactr hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [impactr Config](https://impactr.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/impactr` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
impactr, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde impactr şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa impactr, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `impactr mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından impactr, belirteçleri `~/.local/share/impactr/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra impactr'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi impactr yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve impactr'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.

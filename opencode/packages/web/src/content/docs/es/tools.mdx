...
Las herramientas permiten que LLM realice acciones en su código base. Impactr viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Impactr.
...
:::note
Esta herramienta solo está disponible cuando `IMPACTR_EXPERIMENTAL_LSP_TOOL=true` (o `IMPACTR_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Impactr o cuando la variable de entorno `IMPACTR_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Impactr:

```bash
IMPACTR_ENABLE_EXA=1 impactr
```

...
Puede agregar herramientas externas a Impactr utilizando el _Model Context Protocol_ o MCP. Impactr admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Impactr Config](https://impactr.dev/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/impactr`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Impactr maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Impactr:
...
Si el servidor requiere autenticación, Impactr le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `impactr mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Impactr almacenará los tokens de forma segura en `~/.local/share/impactr/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Impactr, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Impactr como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Impactr a su cuenta Sentry.

---
title: Complementos
description: Escriba sus propios complementos para extender Impactr.
---

Los complementos le permiten extender Impactr al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Impactr.
...
- `.impactr/plugins/` - Complementos a nivel de proyecto
- `~/.config/impactr/plugins/` - Complementos globales
...
```json title="impactr.json"
{
  ...
  "plugin": ["impactr-helicone-session", "impactr-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/impactr/node_modules/`.
...
1. Configuración global (`~/.config/impactr/impactr.json`)
2. Configuración del proyecto (`impactr.json`)
3. Directorio global de complementos (`~/.config/impactr/plugins/`)
4. Directorio de complementos del proyecto (`.impactr/plugins/`)
...
```json title=".impactr/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Impactr ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".impactr/plugins/my-plugin.ts"
...
```js title=".impactr/plugins/example.js"
...
- `client`: Un cliente SDK impactr para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar impactr.
...
```js title=".impactr/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "impactr"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Impactr, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que impactr lea archivos `.env`:

```javascript title=".impactr/plugins/env-protection.js"
...
```javascript title=".impactr/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a impactr:

```ts title=".impactr/plugins/custom-tools.ts"
import { type Plugin, tool } from "@impactr-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que impactr puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para impactr junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://impactr.dev/docs/sdk) para obtener más detalles.
...
```ts title=".impactr/plugins/compaction.ts"
import type { Plugin } from "@impactr-ai/plugin"
...
```ts title=".impactr/plugins/custom-compaction.ts"
import type { Plugin } from "@impactr-ai/plugin"

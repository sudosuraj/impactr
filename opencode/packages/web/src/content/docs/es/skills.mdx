...
Las habilidades del agente permiten a Impactr descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Impactr busca estas ubicaciones:

- Configuración del proyecto: `.impactr/skills/<name>/SKILL.md`
- Configuración global: `~/.config/impactr/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Impactr sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.impactr/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/impactr/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.impactr/skills/git-release/SKILL.md` así:
...
Impactr enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `impactr.json`:
...
**Para agentes integrados** (en `impactr.json`):

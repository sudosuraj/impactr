---
title: Plugin
description: Scrivi plugin per estendere Impactr.
---

I plugin ti permettono di estendere Impactr agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Impactr.
...
- `.impactr/plugins/` - plugin a livello progetto
- `~/.config/impactr/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/impactr/node_modules/`.
...
1. Config globale (`~/.config/impactr/impactr.json`)
2. Config di progetto (`impactr.json`)
3. Directory plugin globale (`~/.config/impactr/plugins/`)
4. Directory plugin di progetto (`.impactr/plugins/`)
...
Impactr esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di impactr per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere impactr.
...
:::note
Se usi l'app desktop di Impactr, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a impactr di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a impactr:
...
L'helper `tool` crea uno strumento personalizzato che impactr può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in impactr insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://impactr.dev/docs/sdk) per i dettagli.

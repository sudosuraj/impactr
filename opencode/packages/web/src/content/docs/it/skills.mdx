...
Le skill degli agenti permettono a Impactr di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Impactr cerca in queste posizioni:

- Config di progetto: `.impactr/skills/<name>/SKILL.md`
- Config globale: `~/.config/impactr/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Impactr risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.impactr/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/impactr/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.impactr/skills/git-release/SKILL.md` cosi':
...
Impactr elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `impactr.json`:
...
**Per agenti integrati** (in `impactr.json`):

...
Puoi aggiungere strumenti esterni a Impactr usando il _Model Context Protocol_, o MCP. Impactr supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Impactr Config](https://impactr.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/impactr`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Impactr gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Impactr:
...
Se il server richiede autenticazione, Impactr ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `impactr mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Impactr memorizzera' i token in modo sicuro in `~/.local/share/impactr/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Impactr insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Impactr come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Impactr al tuo account Sentry.

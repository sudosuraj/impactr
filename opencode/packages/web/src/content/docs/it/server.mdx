---
title: Server
description: Interagisci con il server impactr via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `impactr serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client impactr.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `impactr.local` |
...
Imposta `IMPACTR_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `impactr`, oppure imposta `IMPACTR_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `impactr serve` sia per `impactr web`.
...
Quando esegui `impactr` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server impactr per interagire con impactr in modo programmatico.
:::

Questa architettura permette a impactr di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `impactr serve` per avviare un server standalone. Se la TUI di impactr e gia in esecuzione, `impactr serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Impactr.
...
Il server impactr espone le seguenti API.

---
title: Server
description: Interact with impactr server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `impactr serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an impactr client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `impactr.local` |
...
Set `IMPACTR_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `impactr`, or set `IMPACTR_SERVER_USERNAME` to override it. This applies to both `impactr serve` and `impactr web`.
...
When you run `impactr` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the impactr server to interact with impactr programmatically.
:::

This architecture lets impactr support multiple clients and allows you to interact with impactr programmatically.

You can run `impactr serve` to start a standalone server. If you have the
impactr TUI running, `impactr serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Impactr [IDE](/docs/ide) plugins.
...
The impactr server exposes the following APIs.

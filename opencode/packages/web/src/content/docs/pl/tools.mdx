...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Impactr posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Impactr.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `IMPACTR_EXPERIMENTAL_LSP_TOOL=true` (lub `IMPACTR_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Impactr lub gdy zmienna środowiskowa `IMPACTR_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Impactr:

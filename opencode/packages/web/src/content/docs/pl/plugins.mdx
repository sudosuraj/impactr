---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć impactr.
---

Wtyczki umożliwiające wyłączenie impactr poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania impactr.
...
- `.impactr/plugins/` - Wtyczki na poziomie projektu
- `~/.config/impactr/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/impactr/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/impactr/impactr.json`)
2. Konfiguracja projektu (`impactr.json`)
3. Globalny katalog wtyczek (`~/.config/impactr/plugins/`)
4. Katalog wtyczek projektu (`.impactr/plugins/`)
...
impactr uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z impactr do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do impactr.
...
:::note
Jeśli korzystasz z aplikacji komputerowej impactr, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj impactr czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do impactr:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod impactr. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://impactr.dev/docs/sdk).

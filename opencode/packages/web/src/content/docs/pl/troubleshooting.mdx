...
Aby debugować problemy z impactr, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\impactr\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `impactr --log-level DEBUG`.

---

## Przechowywanie danych

impactr przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/impactr/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\impactr`
...
impactr Desktop uruchamia w tle lokalny serwer impactr (sidecar `impactr-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Impactr` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/impactr/impactr.jsonc` (lub `~/.config/impactr/impactr.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/impactr/impactr.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\impactr\impactr.jsonc`
...
impactr może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/impactr/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\impactr\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby impactr mógł ją odbudować.

1. Całkowicie zamknij impactr Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/impactr`
- **Linux**: usuń `~/.cache/impactr` (lub uruchom `rm -rf ~/.cache/impactr`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\impactr`

3. Uruchom ponownie impactr Desktop.

---

### Napraw problemy z połączeniem z serwerem

impactr Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `impactr.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `IMPACTR_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `IMPACTR_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows impactr Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami impactr.

---

### Brak powiadomień

impactr Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla impactr w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie impactr Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji impactr Desktop):

- `impactr.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `impactr.global.dat` i `impactr.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `impactr upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `impactr models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\impactr`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. impactr dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\impactr`

2. Uruchom ponownie kod impactr, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to impactr do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
impactr wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.

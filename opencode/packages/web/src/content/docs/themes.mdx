...
With Impactr you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Impactr uses our own `impactr` theme.
...
Impactr comes with several built-in themes.
...
- Want Impactr to match their terminal's appearance
...
Impactr supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/impactr/themes/*.json` or `$XDG_CONFIG_HOME/impactr/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.impactr/themes/*.json`
4. **Current working directory** - Defined in `./.impactr/themes/*.json`

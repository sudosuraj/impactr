...
import { Tag } from "@impactr-ai/ui/v2/badge-v2"
...
import { wslImpactrAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.impactrChecks[item.config.distro]
          const impactrAction = () => wslImpactrAction(check())
          const busy = () => wsl.data?.job?.kind === "install-impactr" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={impactrAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installImpactr(item.config.distro))}
                    >

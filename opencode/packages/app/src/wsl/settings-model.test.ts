...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when impactr needs attention", () => {
    expect(wslImpactrAction(undefined)).toBeUndefined()
    expect(
      wslImpactrAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install impactr")
    expect(
      wslImpactrAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/impactr",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when impactr is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        impactrChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.impactr/bin/impactr",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installImpactr" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-impactr")
  })

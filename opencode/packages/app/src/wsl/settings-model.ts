...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-impactr" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslImpactrAction(check?: WslImpactrCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install impactr"
  if (check.matchesDesktop === false) return "Update impactr"
}
...
export function addServerViewModel(input: {
  ...
  const impactrCheck = selectedDistro ? (state?.impactrChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.impactrChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateImpactr" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.impactrMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installImpactr" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  impactrCheck: WslImpactrCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingImpactr =
    input.state?.job?.kind === "install-impactr" && input.state.job.distro === input.selectedDistro
  if (!ready || probingImpactr) {
    return {
      variant: "contrast",
  ...
  if (!addServerImpactrReady(input.impactrCheck)) {
    const update = !!input.impactrCheck?.resolvedPath && input.impactrCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingImpactr
        ? { key: "wsl.onboarding.updatingImpactr" }
        : update
          ? { key: "wsl.onboarding.updateImpactr" }
          : { key: "wsl.onboarding.installImpactr" },
      disabled: !!input.state?.job || input.adding,
      action: "install-impactr",
      loading: installingImpactr,
...
function addServerImpactrReady(check: WslImpactrCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.impactrChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.impactrChecks[item.name]) return [`impactr:${item.name}`]
    return []
  })

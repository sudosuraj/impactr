...
export class ConfigService extends Context.Service<ConfigService, Config>()("@impactr/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.IMPACTR_WEBSEARCH_PROVIDER === "exa" || process.env.IMPACTR_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.IMPACTR_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("IMPACTR_EXPERIMENTAL") || truthy("IMPACTR_ENABLE_EXA") || truthy("IMPACTR_EXPERIMENTAL_EXA"),
    enableParallel: truthy("IMPACTR_ENABLE_PARALLEL") || truthy("IMPACTR_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `impactr/${InstallationVersion}`,

...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@impactr/acme")).toBe("@impactr/acme")
    expect(Npm.sanitize("@impactr/acme@1.0.0")).toBe("@impactr/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/impactr/acme.git"
    const expected = win ? "acme@git+https_//github.com/impactr/acme.git" : spec

...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("impactr-find")
  api.delete("impactr-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("impactr-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("impactr-find", new Highlight(...rest))
    return true
  }

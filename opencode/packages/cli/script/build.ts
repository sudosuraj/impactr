...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      IMPACTR_CLI_NAME: `'${binary}'`,
      IMPACTR_MODELS_DEV: modelsData,
      IMPACTR_CHANNEL: `'${Script.channel}'`,
      IMPACTR_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@impactr-ai/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/impactr.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,

...
import { getImpactrModeStack, IMPACTR_BASE_MODE, ImpactrKeymapProvider, registerImpactrKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerImpactrKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when impactr mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getImpactrModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getImpactrModeStack(keymap).push("autocomplete")

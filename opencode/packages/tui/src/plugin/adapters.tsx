import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@impactr-ai/plugin/tui"
...
import type { useImpactrKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useImpactrKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getImpactrModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getImpactrModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {

...
**The fix pattern (apply this going forward, not just to tools):**
1. When a tool/capability's core logic is pure (no session/permission/framework dependency) — a scoring function, a spec list, a graph algorithm — put it **once** under `packages/core/src/<domain>/` (not inside `src/tool/`) and have **both** the CLI wrapper and the hosted wrapper import that same module. Established examples: `packages/core/src/attack-graph/chains.ts` (exploit-chain finding, shared by both `attack_graph` tools) and `packages/core/src/technique/specs.ts` (the technique engine list — which binaries exist, their args — shared by both `technique` tools).
2. Only the thin wrapper (Schema/Input shape, permission asserts, session-vs-hosted-DB branching) is allowed to differ between the two lineages, because that part is legitimately tied to `SessionV1` vs V2.
3. **This applies beyond tools.** The same duality exists for agent definitions — `packages/impactr/src/agent/agent.ts` + `prompt/*.txt` (CLI: `attack`, `recon`, `enumerate`, `exploit`, `report`) has a hosted V2 counterpart that must be independently checked for the same prompts/permissions/tool allow-lists. Do not assume the CLI's agent behavior (e.g. the `attack` orchestrator's deny-by-default, delegate-only permission model) is mirrored on the hosted side — verify it directly, the same way the tool audit above did: read both, diff them, don't trust that "it typechecks" means "it does the same thing."
4. When you add a **new** pentest capability, wire it into **both** lineages in the same change, even if the hosted side isn't "launched" yet — deferring the hosted half is exactly how the gaps above accumulated.

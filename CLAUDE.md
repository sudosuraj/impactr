...
**Permission *action names* must match what the tool actually asserts, not just what looks parallel (2026-07 regression, caught by review before merge):** giving hosted `enumerate` a real deny-by-default ruleset (see above) briefly broke it completely — every technique tool call was silently denied. Cause: the shared `ENUMERATE` intent lists individual tool names (`scan_ports`, `enumerate_subdomains`, …), matching how the CLI gates permissions (by registered tool name). But hosted's `technique.ts` asserted a single hardcoded `action: "technique"` for every scanner, not `spec.name` — a pre-existing mismatch that was invisible while hosted was allow-all, and only became a real bug once deny-by-default was real. Fixed by asserting `action: spec.name` in `packages/core/src/tool/technique.ts`, matching the CLI. **The general lesson: when locking down a hosted agent's permissions, verify against the actual `permission.assert({ action: ... })` call in each tool it's supposed to use — not against the tool's registered name or the CLI's equivalent — since the two lineages' tools don't always assert the same action string even when they share a name.**
...
**CLI per-engagement workspace isolation (`packages/impactr/src/cli/run-directory.ts`) interacts with `--continue`/`--session` — get this wrong and resumption silently starts a new session instead (2026-07 regression, caught by a second round of automated PR review):** `impactr run` without an explicit `--dir` mints a fresh, isolated `~/.local/share/impactr/engagements/<ulid>/` per launch so concurrent engagements can't collide on the same filesystem state. But session lookup (`Session.list`/`Session.get` in `packages/impactr/src/session/session.ts`) is scoped to `ctx.project.id`, which is derived from the current working directory. A resuming launch (`--continue` or `--session <id>`, no `--dir`) that still mints a brand-new empty directory therefore resolves to a project with zero sessions — the CLI then silently falls through to creating a fresh session instead of resuming the one the user asked for, with no error. Fixed in `resolveRunDirectory` via a `preferCwd` option (`Boolean(args.continue || args.session)`): when resuming and no `--dir` was given, it resolves to the launch cwd instead of isolating into a new workspace (still passed through the same `isInsideImpactrSource` guard). Same change also fixed two related, lower-severity findings from the same review pass: `defaultEngagementDirectory()` now registers a `process.on("exit", ...)` cleanup that removes the freshly-minted directory if it's still empty, so aborted/invalid launches don't leave stray folders under `engagements/`; and both the `directory:` cmd option and the handler's own chdir now resolve `--dir` through one shared `resolveLaunchRoot()` helper instead of drifting between `process.cwd()` and `$PWD`-aware `root`. **General lesson: any "mint a fresh directory/workspace per invocation" feature must special-case every other flag that identifies state by *current directory/project* rather than by explicit ID — grep for `process.cwd()`/`ctx.project.id` consumers before shipping directory-minting behavior, not after a review catches it. Also: when a fix lands as a reply comment on a PR, verify the commit it references actually reached the branch you're about to merge/close — a fix can be pushed to a branch after its PR is already closed, orphaning real work that a later consolidation would otherwise silently drop.**

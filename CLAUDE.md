...
**CLI per-engagement workspace isolation (`packages/impactr/src/cli/run-directory.ts`) interacts with `--continue`/`--session` — get this wrong and resumption silently starts a new session instead (2026-07 regression, caught by a second round of automated PR review, after the first fix had already landed and the PR had been merged/closed):** `impactr run` without an explicit `--dir` mints a fresh, isolated `~/.local/share/impactr/engagements/<ulid>/` per launch so concurrent engagements can't collide on the same filesystem state. But session lookup (`Session.list`/`Session.get` in `packages/impactr/src/session/session.ts`) is scoped to `ctx.project.id`, which is derived from the current working directory. A resuming launch (`--continue` or `--session <id>`, no `--dir`) that still mints a brand-new empty directory therefore resolves to a project with zero sessions — the CLI then silently falls through to creating a fresh session instead of resuming the one the user asked for, with no error. Fixed by threading a `resuming` flag (`Boolean(args.continue || args.session)`) into `resolveRunDirectory`: when resuming and no `--dir` was given, it now falls back to the launch cwd (the pre-isolation behavior) instead of isolating into a new workspace. **General lesson: any "mint a fresh directory/workspace per invocation" feature must special-case every other flag that identifies state by *current directory/project* rather than by explicit ID — grep for `process.cwd()`/`ctx.project.id` consumers before shipping directory-minting behavior, not after a review catches it.**

...
Before running anything, call the `get_scope` tool to fetch the authorized target scope and
exclusions for this engagement from the tracked authorization record — never from an ad hoc file
or your own assumption. If it reports no tracked engagement or a non-active status, stop and
confirm authorization with the operator before proceeding.
...
- **DO NOT** scan targets that have not been explicitly authorized by `get_scope`'s tracked record.

...
import type { AssistantMessage, Message, ImpactrClient, SessionMessageResponse } from "@impactr-ai/sdk/v2"
...
export const AuthMethodID = "impactr-login"
...
export class Service extends Context.Service<Service, Interface>()("@impactr/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `impactr auth login` in the terminal",
      name: "Login with impactr",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: ImpactrClient) {
...
function makeUsageService(sdk: ImpactrClient) {
...
async function loadDirectorySnapshot(sdk: ImpactrClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, impactr provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const impactrProvider = providers[ProviderV2.ID.make("impactr")]
  const impactrModel = impactrProvider ? Provider.sort(Object.values(impactrProvider.models))[0] : undefined
  if (impactrProvider && impactrModel) return { providerID: impactrProvider.id, modelID: impactrModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Impactr service failure", service })
}

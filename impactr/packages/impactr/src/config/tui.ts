...
import { isRecord } from "@impactr-ai/tui/util/record"
...
import { TuiKeybind } from "@impactr-ai/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@impactr/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old impactr.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.impactr`
  // folders between cwd and home, and IMPACTR_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.IMPACTR_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit IMPACTR_TUI_CONFIG override, if set.
  if (Flag.IMPACTR_TUI_CONFIG) {
    const configFile = Flag.IMPACTR_TUI_CONFIG
  ...
  // 4. `.impactr` directories (and IMPACTR_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".impactr") || dir === Flag.IMPACTR_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".impactr") && dir !== Flag.IMPACTR_CONFIG_DIR) continue

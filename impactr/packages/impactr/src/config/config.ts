...
import { InstallationLocal, InstallationVersion } from "@impactr-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@impactr-ai/core/v1/config/console-state"
import { FSUtil } from "@impactr-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@impactr-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@impactr-ai/core/v1/config/config"
import { RemoteAuthError } from "@impactr-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@impactr-ai/core/v1/config/permission"
...
function globalConfigFile() {
  const candidates = ["impactr.jsonc", "impactr.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.IMPACTR_CONFIG && !Flag.IMPACTR_CONFIG_DIR && !Flag.IMPACTR_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "impactr.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "impactr.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "IMPACTR_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/impactr`
        ...
        if (Flag.IMPACTR_CONFIG) {
          yield* merge(Flag.IMPACTR_CONFIG, yield* loadFile(Flag.IMPACTR_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.IMPACTR_CONFIG })
        }

        if (!Flag.IMPACTR_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("impactr", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.IMPACTR_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from IMPACTR_CONFIG_DIR", { path: Flag.IMPACTR_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".impactr") || dir === Flag.IMPACTR_CONFIG_DIR) {
            for (const file of ["impactr.json", "impactr.jsonc"]) {
          ...
          result.agent = mergeDeep(result.agent ?? {}, yield* Effect.promise(() => ConfigAgent.load(dir))) as Info["agent"]
          result.agent = mergeDeep(result.agent ?? {}, yield* Effect.promise(() => ConfigAgent.loadMode(dir))) as Info["agent"]
          // Auto-discovered plugins under `.impactr/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.IMPACTR_CONFIG_CONTENT) {
          const source = "IMPACTR_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.IMPACTR_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from IMPACTR_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["IMPACTR_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("IMPACTR_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.IMPACTR_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.IMPACTR_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("IMPACTR_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.IMPACTR_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.IMPACTR_DISABLE_PRUNE) {

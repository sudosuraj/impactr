...
import { Shell } from "@impactr-ai/core/shell"
...
export const ShellTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const shellEnv = Effect.fn("ShellTool.shellEnv")(function* (ctx: Tool.Context, cwd: string, customEnv?: Record<string, string>) {
            ...
            Effect.gen(function* () {
              ...
              if (params.background) {
                ...
                const detachedCtx: Tool.Context = {
                  ...ctx,
                  abort: detachedController.signal,
                  metadata: () => Effect.void, // ignore metadata updates in background
                }
                      ...
                      parts: [
                        {
                          type: "text",
                          synthetic: true,
                          text: `<background_shell_result command="${params.command}" state="${state}">\n${text}\n</background_shell_result>`,
                        },
                      ],
                    })
                ...
                const jobInfo = yield* background.start({
                  type: "shell",
                  title: params.command,
                  run: run({ shell, command: params.command, cwd, env, timeout }, detachedCtx).pipe(
                    Effect.map((r) => r.output),
                    Effect.onExit((exit) => Effect.sync(() => detachedController.abort()))
                  )
                })

                // Notify when done
                yield* background.wait({ id: jobInfo.id }).pipe(

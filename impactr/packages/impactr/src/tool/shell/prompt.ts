...
import { PositiveInt } from "@impactr-ai/core/schema"
...
export function parameterSchema() {
  return Schema.Struct({
    ...
    env: Schema.optional(Schema.Record(Schema.String, Schema.String)).annotate({
      description: "Optional key-value pairs of environment variables to set temporarily for this command execution.",
    }),
    background: Schema.optional(Schema.Boolean).annotate({
      description: "If true, runs the command asynchronously in the background. The tool will return immediately with a task ID. You will be notified via pub/sub when it finishes.",
    }),
  })
}

...
import { ImpactrHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for impactr serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ImpactrHttpApi)
...
export const PublicApi = ImpactrHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "impactr",
    version: "1.0.0",
    description: "impactr api",

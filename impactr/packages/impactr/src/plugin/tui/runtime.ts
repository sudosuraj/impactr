...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@impactr-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@impactr-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@impactr-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@impactr-ai/tui/context/theme"
import { Global } from "@impactr-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@impactr-ai/core/util/flock"
import { Flag } from "@impactr-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@impactr-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@impactr-ai/core/v1/config/plugin"
import { createCommandShim } from "@impactr-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@impactr-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".impactr"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".impactr", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".impactr", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.IMPACTR_PURE ? [] : pluginOrigins
    if (Flag.IMPACTR_PURE && pluginOrigins.length) {
    }

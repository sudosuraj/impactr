...
import type { LLMClientShape } from "@impactr-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("impactr"))
    return { type: "unsupported", reason: "provider is not openai, impactr, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @impactr-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains impactr-owned. The native runtime only adapts
      // the @impactr-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({

...
//   2. Interactive local (`impactr --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`impactr --mini --attach`): connects to a running
//      impactr server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running impactr server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to IMPACTR_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to IMPACTR_SERVER_USERNAME or 'impactr')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      if (engagementStore && args.target) {
        ...
        // A --dir the operator explicitly gave is a real, stable project identity, so dedup still
        // requires matching it exactly. Without --dir, targetDirectory is a fresh auto-generated
        // per-launch workspace (see RunDirectory.isAutoGeneratedEngagementDirectory) with no identity
        // of its own — requiring it to match would mean the *same* target/scope launched twice never
        // reuses its engagement record, since every launch mints a different workspace.
        const dedupeDirectory = RunDirectory.isAutoGeneratedEngagementDirectory(targetDirectory) ? null : targetDirectory
        const { engagement, reused } = await Effect.runPromise(
          Effect.gen(function* () {
            const existing = EngagementStore.findReusable(yield* engagementStore.list(), {
              directory: dedupeDirectory,
              target,
              scope,
              exclusions,
            })
      ...
      async function share(sdk: ImpactrClient, sessionID: string) {
      ...
      async function current(sdk: ImpactrClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: ImpactrClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: ImpactrClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: ImpactrClient) {
        ...
        async function loop(client: ImpactrClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createImpactrClient({
        baseUrl: "http://impactr.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)

// Shared type vocabulary for the direct interactive mode (`impactr --mini`).
//
...
import type { ImpactrClient, PermissionRequest, QuestionRequest, ToolPart } from "@impactr-ai/sdk/v2"
import type { TuiConfig } from "@impactr-ai/tui/config"
...
type PromptModel = Parameters<ImpactrClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<ImpactrClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<ImpactrClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<ImpactrClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<ImpactrClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<ImpactrClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<ImpactrClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<ImpactrClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<ImpactrClient["question"]["reject"]>[0]

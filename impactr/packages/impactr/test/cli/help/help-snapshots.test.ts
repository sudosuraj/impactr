...
// terminal sizes. The default impactr tui command is excluded —
// `impactr --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `impactr --help` prints today;
...
// top-level help on `--help` and exits 1; not a real impactr command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("impactr CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ impactr }) =>
      Effect.gen(function* () {
        const topLevel = yield* impactr.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* impactr.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`impactr ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`impactr ${argv.join(" ")} --help`)
        }

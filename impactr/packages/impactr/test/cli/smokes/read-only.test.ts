...
describe("impactr read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // IMPACTR_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ impactr }) =>
      Effect.gen(function* () {
        const r = yield* impactr.spawn(["mcp", "list"])
        impactr.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ impactr }) =>
      Effect.gen(function* () {
        const r = yield* impactr.spawn(["providers", "list"])
        impactr.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ impactr }) =>
      Effect.gen(function* () {
        const r = yield* impactr.spawn(["models"])
        impactr.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ impactr }) =>
      Effect.gen(function* () {
        const r = yield* impactr.spawn(["agent", "list"])
        impactr.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh IMPACTR_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ impactr }) =>
      Effect.gen(function* () {
        const r = yield* impactr.spawn(["session", "list"])
        impactr.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ impactr }) =>
      Effect.gen(function* () {
        const r = yield* impactr.spawn(["stats"])
        impactr.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under IMPACTR_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ impactr }) =>
      Effect.gen(function* () {
        const r = yield* impactr.spawn(["db", "path"])
        impactr.expectExit(r, 0, "db path")

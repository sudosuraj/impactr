...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ impactr }) =>
    Effect.gen(function* () {
      const result = yield* impactr.spawn(["--replay-limit", "10"])

      impactr.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ impactr }) =>
    Effect.gen(function* () {
      const result = yield* impactr.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ impactr }) =>
    Effect.gen(function* () {
      const result = yield* impactr.spawn(["--mini", "--port", "4096"])

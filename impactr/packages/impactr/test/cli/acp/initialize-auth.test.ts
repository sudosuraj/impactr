...
describe("impactr acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ impactr }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ impactr }))
        ...
        expect(initialized.agentInfo?.name).toBe("Impactr")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ impactr }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ impactr })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("impactr-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "impactr-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.IMPACTR_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ impactr }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("impactr-login")

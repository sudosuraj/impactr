...
import { ImpactrClient, type GlobalEvent } from "@impactr-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<ImpactrClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<ImpactrClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<ImpactrClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<ImpactrClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<ImpactrClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://impactr.test"),
...
function sdk(
  input: {
    ...
    subscribe?: ImpactrClient["event"]["subscribe"]
    globalEvent?: ImpactrClient["global"]["event"]
    promptAsync?: ImpactrClient["session"]["promptAsync"]
    status?: ImpactrClient["session"]["status"]
    messages?: ImpactrClient["session"]["messages"]
    children?: ImpactrClient["session"]["children"]
    permissions?: ImpactrClient["permission"]["list"]
    questions?: ImpactrClient["question"]["list"]
  } = {},
) {
  const client = new ImpactrClient()

  const subscribe: ImpactrClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: ImpactrClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: ImpactrClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: ImpactrClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: ImpactrClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: ImpactrClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: ImpactrClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: ImpactrClient["question"]["list"] = input.questions ?? (() => ok([]))

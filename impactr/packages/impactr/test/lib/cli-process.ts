// Subprocess test harness for the impactr CLI. Spawns the real binary against
...
//
// Configuration flows through impactr's built-in test affordances:
//   - IMPACTR_CONFIG_CONTENT      : provider config inline, no files to find
//   - IMPACTR_TEST_HOME           : pins os.homedir() → tmpdir
//   - IMPACTR_DISABLE_PROJECT_CONFIG : skip walking up for impactr.json
//   - IMPACTR_PURE                : skip external plugin discovery + install
//   - IMPACTR_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `impactr.run` is fully wired. The shape supports adding more
// builders (`impactr.serve(opts)`, `impactr.acp(opts)`, `impactr.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of ImpactrCli.
...
const impactrRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(impactrRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    IMPACTR_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    IMPACTR_CONFIG_CONTENT: configJson,
    IMPACTR_DISABLE_PROJECT_CONFIG: "1",
    IMPACTR_PURE: "1",
    IMPACTR_DISABLE_AUTOUPDATE: "1",
    IMPACTR_DISABLE_AUTOCOMPACT: "1",
    IMPACTR_DISABLE_MODELS_FETCH: "1",
    IMPACTR_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `impactr run`. New flags should
...
// `impactr serve` is a long-lived process — it never exits on its own.
...
// `impactr acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type ImpactrCli = {
  ...
  // Spawn `impactr serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `impactr acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("impactr.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          IMPACTR_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("impactr.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("impactr.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "impactr server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `impactr serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("impactr.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const impactr: ImpactrCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, impactr })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `impactr.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "impactr") {
...
// every call site. Use this for any test that needs the impactr CLI fixture.
//
...
// resources (e.g. `impactr.serve`) without an extra `Effect.scoped` wrapper —

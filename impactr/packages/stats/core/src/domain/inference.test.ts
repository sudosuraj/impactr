...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve impactr route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "impactr")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "impactr")).toBe("openai")
    expect(statProvider("big-pickle", "", "impactr")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "impactr"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep impactr as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "impactr"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "impactr"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep impactr or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "impactr"), country: "US" })).toMatchObject([

...
export const IMPACTR_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const IMPACTR_MODE_KEY = "impactr.mode"

export const ImpactrKeymapProvider = KeymapProvider
export const useImpactrKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type ImpactrModeStack = ReturnType<typeof createImpactrModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, ImpactrModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createImpactrModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(IMPACTR_MODE_KEY, IMPACTR_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(IMPACTR_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(IMPACTR_MODE_KEY, stack.at(-1)?.mode ?? IMPACTR_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? IMPACTR_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(IMPACTR_MODE_KEY, undefined)
...
export function useImpactrModeStack() {
  return getImpactrModeStack(useImpactrKeymap())
}

export function getImpactrModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Impactr mode stack is not registered for this keymap")
  return value
}
...
export function registerImpactrKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createImpactrModeStack(keymap)

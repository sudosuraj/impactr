...
import { type Session } from "@impactr-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("impactr://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("impactr://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("impactr://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("impactr://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("impactr://open-project")).toBeUndefined()
    expect(parseDeepLink("impactr://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "impactr://open-project?directory=/a",
      "impactr://other?directory=/b",
      "impactr://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("impactr://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("impactr://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("impactr://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("impactr://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "impactr://new-session?directory=/a",
      "impactr://open-project?directory=/b",
      "impactr://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __IMPACTR__: {
        deepLinks: ["impactr://open-project?directory=/a"],
      },
    } as unknown as Window & { __IMPACTR__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["impactr://open-project?directory=/a"])

  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://impactr.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://impactr.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://impactr.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://impactr.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://impactr.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://impactr.example.com"),
      ),
    ).toEqual({

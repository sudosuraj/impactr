...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-impactr-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-impactr-directory"] ? decode(request.headers["x-impactr-directory"]) : process.cwd())

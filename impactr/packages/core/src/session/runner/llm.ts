...
import { MemoryEmbedder } from "../../memory/embedder"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const compaction = SessionCompaction.make({
      ...
      onSummary: ({ sessionID, summary, sourceText }) =>
        Effect.all([
          MemoryEmbedder.indexCompactionSummary(db, { projectId, sessionId: sessionID, summary }),
          MemoryEmbedder.index(db, {
            projectId,
            sessionId: sessionID,
    ...
    const runTurnAttempt = Effect.fn("SessionRunner.runTurn")(function* (
      ...
      // Budget check
      const isExhausted = yield* budget.isExhausted()
      if (isExhausted) {
      ...
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          if (stepSettlement && !publisher.hasProviderError()) {
            yield* budget.consumeTokens(stepSettlement.tokens.input + stepSettlement.tokens.output, 0)
            yield* budget.consumeRequest()
    ...
    const run = Effect.fn("SessionRunner.run")(function* (input: {
      ...
      // Recover hypotheses orphaned in "processing" by a prior interrupted drain.
      yield* hypothesisQueue.reclaimStale(input.sessionID)
              ...
              if (session && session.agent === "build") {
                ...
                if (hasPackageJson) {
                  ...
                  if (packageJson?.scripts?.test && packageJson.scripts.test !== "echo \"Error: no test specified\" && exit 1") {
                    ...
                    testCommandStr = hasPackageLock ? "npm run test" : "bun run test"
                  }
                }
                if (testCommandStr) {
                  const testCommand = ChildProcess.make(testCommandStr, [], {
                    cwd: location.directory,
                    stdin: "ignore",
                    detached: process.platform !== "win32",
                    forceKillAfter: Duration.seconds(3),
                  })
                  const testResult = yield* appProcess.run(testCommand, {
                    combineOutput: true,
                  ...
                  if (testResult && testResult.exitCode !== 0) {
                    fixAttempts++
                    const errorOutput = testResult.stdout.toString("utf8") + testResult.stderr.toString("utf8")
                    const messageID = SessionMessage.ID.create()
                    yield* SessionInput.admit(db, events, {
                      id: messageID,
                      sessionID: input.sessionID,
                      prompt: Prompt.make({
                        text: `Auto-healing CI: The test suite failed after your changes (Attempt ${fixAttempts}/3). Please fix the failures.\n\nCommand: ${testCommandStr}\nExit Code: ${testResult.exitCode}\n\nOutput:\n${errorOutput.slice(0, 5000)}`,
                      }),
                      delivery: "steer",
                    })
                    needsContinuation = true

...
export class Service extends Context.Service<Service, Interface>()("@impactr/v2/Config") {}
  ...
  Effect.gen(function* () {
    ...
    const loadFile = Effect.fnUntraced(function* (filepath: string) {
      ...
      if (filepath.endsWith(".ts")) {
        let code = text.replace(/import\s+[\s\S]*?;/g, "")
        ...
        try {
          ...
          fn(m, m.exports)
          input = m.exports?.default ?? m.exports
        } catch {
          return
        }
      } else {
        const errors: ParseError[] = []
        input = parse(text, errors, { allowTrailingComma: true })
    ...
    // global config, project files, then `.impactr` files.

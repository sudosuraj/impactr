 ...
 *
 * `match.directory: null` means "don't require a directory match" — callers pass this when the
 * candidate directory is itself a per-launch auto-generated engagement workspace (see
 * `RunDirectory.isAutoGeneratedEngagementDirectory`) rather than a real project checkout the operator
 * pointed at explicitly. Auto-generated workspaces are unique on every launch, so requiring equality
 * there would mean the same target/scope authorized twice in a row is never recognized as a repeat and
 * a fresh engagement record piles up on every single run. An explicit --dir is a real, stable identity
 * boundary and still requires an exact match, same as before.
 */
export const findReusable = (
  engagements: readonly LocalEngagement[],
  match: { readonly directory: string | null; readonly target: string; readonly scope: string; readonly exclusions?: readonly string[] },
  ...
  return engagements.find((engagement) => {
    ...
    return (
      ...
      (match.directory === null || engagement.directory === match.directory) &&

...
import type { IntegrationOAuthMethodRegistration } from "@impactr-ai/plugin/v2/effect/integration"
import { define } from "@impactr-ai/plugin/v2/effect/plugin"
...
export const ImpactrPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "impactr",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("ImpactrPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("impactr")
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("impactr", (integration) => {
        integration.name = "impactr"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "impactr", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("impactr")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("impactr")
      ...
      const item = catalog.provider.get(ProviderV2.ID.impactr)
      if (!item) return
      const hasKey = Boolean(process.env.IMPACTR_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("impactr")),

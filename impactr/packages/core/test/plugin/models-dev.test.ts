...
import { ModelsDevPlugin } from "@impactr-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.IMPACTR_MODELS_PATH,
          disabled: Flag.IMPACTR_DISABLE_MODELS_FETCH,
        }
        Flag.IMPACTR_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.IMPACTR_MODELS_PATH = previous.path
          Flag.IMPACTR_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})

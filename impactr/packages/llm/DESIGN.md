...
> current private `@impactr-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@impactr-ai/ai`
...
- Secondary audience: Impactr and other durable agent runtimes
...
import { LLM } from "@impactr-ai/ai"
import { OpenAI } from "@impactr-ai/ai/providers/openai"
...
```ts
import { Message } from "@impactr-ai/ai"
...
import { LLM, Tool } from "@impactr-ai/ai"
...
Impactr and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@impactr-ai/ai/promise"
import { OpenAI } from "@impactr-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@impactr-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@impactr-ai/ai/provider"
...
@impactr-ai/ai/promise
  Promise/AsyncIterable LLM facade

@impactr-ai/ai/schema
  serializable domain schemas

@impactr-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@impactr-ai/ai/providers/openai
@impactr-ai/ai/providers/anthropic
@impactr-ai/ai/providers/google
...
```
...
Impactr should migrate to `generateTurn` / `streamTurn`, preserving its durable

...
export async function installWslImpactr(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
...
export async function resolveWslImpactr(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.impactr/bin/impactr" ]; then printf "%s\\n" "$HOME/.impactr/bin/impactr"; fi',
        distro,
        opts,
      )
    ).stdout,

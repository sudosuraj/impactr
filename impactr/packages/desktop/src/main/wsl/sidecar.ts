...
import { type WslCommandLine, resolveWslImpactr, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const impactr = await resolveWslImpactr(distro)
  if (!impactr) throw new Error(`Impactr is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "impactr"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export IMPACTR_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export IMPACTR_CLIENT=desktop",
    `export IMPACTR_SERVER_USERNAME=${shellEscape(username)}`,
    `export IMPACTR_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(impactr)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")

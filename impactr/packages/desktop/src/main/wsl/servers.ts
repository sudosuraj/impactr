...
import { expectImpactrVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveImpactr?: typeof resolveWslImpactr
...
export function createWslServersController(
  ...
  const setImpactrCheck = (distro: string, check: WslImpactrCheck) => {
    setState({
      impactrChecks: {
        ...state.impactrChecks,
        [distro]: check,
  ...
  const checkImpactr = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveImpactr ?? resolveWslImpactr)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return impactrCheck(distro, resolved, version, appVersion)
  }

  const refreshImpactrCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setImpactrCheck(distro, await checkImpactr(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const impactrChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.impactrChecks[distro])
        .map(async (distro) => [distro, await checkImpactr(distro, opts)] as const),
    )
    if (impactrChecks.length) {
      setState({ impactrChecks: { ...state.impactrChecks, ...Object.fromEntries(impactrChecks) } })
    }
  }
  ...
  const refreshImpactrCheckBackground = (id: string, distro: string) => {
    void checkImpactr(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setImpactrCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl impactr check failed", { id, distro, message })
      })
  }

  const refreshImpactrChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkImpactr(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setImpactrCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl impactr check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshImpactrCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installImpactr(name: string) {
      await runJob({ kind: "install-impactr", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslImpactr(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "Impactr installation failed")
        }
        await refreshImpactrCheck(name, { signal: abort.signal })
        expectImpactrVersion(state.impactrChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.impactrChecks, distro) : {}),
      })
    },
...
function impactrCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "impactr is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "impactr is installed but could not run",
    }
  }
  return {
    distro,

...
Com o impactr, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o impactr usa nosso próprio tema `impactr`.
...
O impactr vem com vários temas embutidos.
...
- Querem que o impactr corresponda à aparência do seu terminal
...
O impactr suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/impactr/themes/*.json` ou `$XDG_CONFIG_HOME/impactr/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.impactr/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.impactr/themes/*.json`

...
As habilidades do agente permitem que o impactr descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O impactr pesquisa nesses locais:

- Configuração do projeto: `.impactr/skills/<name>/SKILL.md`
- Configuração global: `~/.config/impactr/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o impactr sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.impactr/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/impactr/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.impactr/skills/git-release/SKILL.md` assim:
...
O impactr lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `impactr.json`:
...
**Para agentes embutidos** (em `impactr.json`):

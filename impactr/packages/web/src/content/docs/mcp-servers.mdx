...
You can add external tools to Impactr using the _Model Context Protocol_, or MCP. Impactr supports both local and remote servers.
...
You can define MCP servers in your [Impactr Config](https://impactr.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/impactr` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Impactr automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Impactr will:
...
If the server requires authentication, Impactr will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `impactr mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Impactr will store the tokens securely in `~/.local/share/impactr/mcp-auth.json`.
...
Your MCPs are available as tools in Impactr, alongside built-in tools. So you can manage them through the Impactr config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Impactr to your Sentry account.

---
title: Eklentiler
description: Impactr'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Impactr'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Impactr'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.impactr/plugins/` - Proje düzeyinde eklentiler
- `~/.config/impactr/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/impactr/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/impactr/impactr.json`)
2. Project config (`impactr.json`)
3. Global eklenti dizini (`~/.config/impactr/plugins/`)
4. Proje eklenti dizini (`.impactr/plugins/`)
...
Impactr bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Impactr SDK istemcisi.
...
Impactr'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Impactr masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Impactr'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Impactr'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Impactr'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Impactr'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://impactr.dev/docs/sdk) bakın.

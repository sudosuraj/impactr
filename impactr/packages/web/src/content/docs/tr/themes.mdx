...
impactr ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak impactr kendi `impactr` temasını kullanır.
...
impactr birden fazla yerleşik temayla gelir.
...
- impactr'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
impactr, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/impactr/themes/*.json` veya `$XDG_CONFIG_HOME/impactr/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.impactr/themes/*.json`
4. **Geçerli çalışma dizini** - `./.impactr/themes/*.json`

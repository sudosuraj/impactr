---
title: Sağlayıcılar
description: impactr'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

impactr, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı impactr yapılandırmanızda yapılandırın.
...
`~/.local/share/impactr/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları impactr'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Impactr Zen, impactr ekibi tarafından sağlanan modellerin bir listesidir.
impactr ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Impactr Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, impactr'u seçin ve [impactr.dev/auth](https://impactr.dev/auth)'ye gidin.
...
impactr'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Impactr Go

Impactr Go, impactr ile iyi çalıştığı test edilmiş ve doğrulanmış, impactr ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Impactr Go`'yu seçin ve [impactr.dev/auth](https://impactr.dev/zen) adresine gidin.
...
Amazon Bedrock'u impactr ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   impactr'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `impactr.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi impactr'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
impactr'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   impactr'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Impactr yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
impactr, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. impactr'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`impactr.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [impactr-gitlab-auth](https://www.npmjs.com/package/impactr-gitlab-auth) ana sayfasında.

##### Yapılandırma

`impactr.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi impactr'la kullanmak için:
...
Google Vertex AI'yi impactr ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları impactr çalıştırılırken ayarlayın.
...
Helicone'un impactr aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her impactr görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [impactr-helicone-session](https://github.com/H2Shami/impactr-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her impactr görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
impactr'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla impactr'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla impactr'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini impactr için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/impactr)'a bakın.
:::
...
Ollama Cloud'u impactr ile kullanmak için:
...
3. impactr'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: impactr'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Impactr Zen, impactr ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Impactr Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Impactr Zen**'i arayın.

   ```txt
   /connect
   ```

3. impactr API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   impactr yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca impactr yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Impactr ile kullanmak için:
...
Modelleri impactr yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı impactr ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `impactr.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları impactr'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `impactr auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için impactr yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, impactr yapılandırmanızdaki kimlikle eşleştiğinden emin olun.

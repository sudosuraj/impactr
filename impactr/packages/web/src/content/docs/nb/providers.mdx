---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Impactr.
---

import config from "../../../../config.mjs"
export const console = config.console

Impactr bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Impactr-konfigurasjonen.
...
i `~/.local/share/impactr/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Impactr
konfig.
...
Impactr Zen er en liste over modeller levert av Impactr-teamet som har vært
testet og verifisert for å fungere godt med Impactr. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Impactr Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Impactr Zen` og gå til [impactr.dev/auth](https://impactr.dev/zen).
...
Det fungerer som alle andre leverandører i Impactr og er helt valgfritt å bruke.

---

## Impactr Go

Impactr Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Impactr-teamet som har vært
testet og verifisert for å fungere godt med Impactr.

1. Kjør kommandoen `/connect` i TUI, velg `Impactr Go`, og gå til [impactr.dev/auth](https://impactr.dev/zen).
...
Slik bruker du Amazon Bedrock med Impactr:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Impactr:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `impactr.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i impactr støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere impactr til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Impactr skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Impactr-konfigurasjonen.
...
for å lagre token i Impactr auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Impactr bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Impactr
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`impactr.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [impactr-gitlab-auth](https://www.npmjs.com/package/impactr-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `impactr.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Impactr:
...
Slik bruker du Google Vertex AI med Impactr:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Impactr.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Impactr, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [impactr-helicone-session](https://github.com/H2Shami/impactr-helicone-session) plugin for automatisk å logge hver Impactr-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Impactr-samtale oppført som en separat økt.
...
Du kan konfigurere Impactr for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Impactr for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Impactr for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Impactr. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/impactr) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Impactr:
...
3. Kopier API-nøkkelen for bruk i Impactr.
...
6. **Viktig**: Før du bruker skymodeller i Impactr, må du hente modellinformasjonen lokalt:
...
Impactr Zen er en liste over testede og verifiserte modeller levert av Impactr-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Impactr Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Impactr Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Impactr API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Impactr-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Impactr-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Impactr:
...
Du kan også tilpasse modeller gjennom Impactr-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Impactr. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `impactr.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Impactr forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `impactr auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Impactr-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Impactr-konfigurasjonen.

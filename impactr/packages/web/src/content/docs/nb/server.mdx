---
title: Tjener
description: Samhandle med Impactr-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `impactr serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Impactr-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `impactr.local` |
...
Sett `IMPACTR_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `impactr` som standard, eller sett `IMPACTR_SERVER_USERNAME` for å overstyre det. Dette gjelder både `impactr serve` og `impactr web`.
...
Når du kjører `impactr` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Impactr-serveren til å samhandle med Impactr programmatisk.
:::

Denne arkitekturen lar Impactr støtte flere klienter og lar deg samhandle med Impactr programmatisk.

Du kan kjøre `impactr serve` for å starte en frittstående server. Hvis du har
Impactr TUI kjører, vil `impactr serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Impactr [IDE](/docs/ide) plugins.
...
impactr-serveren viser følgende APIer.

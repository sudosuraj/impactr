...
Umiejętności agenta pozwalają impactr odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
impactr przeszukuje te lokalizacje:

- Project config: `.impactr/skills/<name>/SKILL.md`
- Global config: `~/.config/impactr/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu impactr przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.impactr/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/impactr/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.impactr/skills/git-release/SKILL.md` w ten sposób:
...
impactr wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `impactr.json`:
...
**Dla agentów wbudowanych** (w `impactr.json`):

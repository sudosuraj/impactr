...
Możesz dodać dodatkowe informacje do impactr za pomocą _Model Context Protocol_, w skrócie MCP. impactr obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [impactr Config](https://impactr.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/impactr`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
impactr automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, impactr:
...
Jeśli serwer wymaga uwierzytelnienia, impactr poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `impactr mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji impactr bezpieczne przechowa tokeny w `~/.local/share/impactr/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w impactr, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie impactr, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć impactr z kontem Sentry.

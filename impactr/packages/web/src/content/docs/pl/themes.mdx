...
Dzięki impactr możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie impactr używa naszego własnego motywu `impactr`.
...
impactr ma kilka wbudowanych motywów.
...
- Chcą, aby Impactr pasował do wyglądu ich terminala
...
impactr obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/impactr/themes/*.json` lub `$XDG_CONFIG_HOME/impactr/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.impactr/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.impactr/themes/*.json`

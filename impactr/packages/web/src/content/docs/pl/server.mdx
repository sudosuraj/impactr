---
title: Serwer
description: Interakcja z serwerem impactr za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `impactr serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient impactr.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `impactr.local` |
...
Ustaw `IMPACTR_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `impactr` lub ustaw `IMPACTR_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `impactr serve`, jak i `impactr web`.
...
Kiedy uruchomisz `impactr`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera impactr do programowej interakcji z impactr.
:::

Ta architektura umożliwia obsługę wielu klientów przez impactr i programową interakcję z kodem otwartym.

Możesz uruchomić `impactr serve`, aby uruchomić samodzielny serwer. Jeśli masz
impactr TUI działa, `impactr serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki impactr [IDE](/docs/ide).
...
Serwer impactr udostępnia następujące interfejsy API.

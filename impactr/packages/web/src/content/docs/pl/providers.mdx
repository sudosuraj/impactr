---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w impactr.
---

import config from "../../../../config.mjs"
export const console = config.console

impactr używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji impactr.
...
w `~/.local/share/impactr/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim impactr
config.
...
Impactr Zen to lista modeli dostarczonych przez zespół impactr, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z impactr. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Impactr Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Impactr Zen` i przejdź do [impactr.dev/auth](https://impactr.dev/zen).
...
Działa jak każdy inny dostawca w impactr i jest całkowicie opcjonalny w użyciu.

---

## Impactr Go

Impactr Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół impactr, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z impactr.

1. Uruchom polecenie `/connect` w TUI, wybierz `Impactr Go` i przejdź do [impactr.dev/auth](https://impactr.dev/zen).
...
Aby używać Amazon Bedrock z impactr:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania impactr:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `impactr.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w impactr nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować impactr do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod impactr działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji impactr.
...
impactr używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować impactr
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`impactr.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [impactr-gitlab-auth](https://www.npmjs.com/package/impactr-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem impactr:
...
Aby używać Google Vertex AI z impactr:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania impactr.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą impactr, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [impactr-helicone-session](https://github.com/H2Shami/impactr-helicone-session), aby automatycznie rejestrować każdą konwersację impactr jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja impactr będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować impactr tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować impactr tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować impactr tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem impactr. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/impactr).
:::
...
Aby korzystać z Ollama Cloud z impactr:
...
3. Skopiuj klucz API do użycia w impactr.
...
6. **Ważne**: Przed użyciem modeli chmurowych w impactr musisz pobrać informacje o modelu lokalnie:
...
Impactr Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół impactr. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Impactr Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Impactr Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API impactr.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji impactr.
...
5. Możesz je także dostosować za pomocą konfiguracji impactr. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z impactr:
...
Możesz także dostosować modele za pomocą konfiguracji impactr. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z impactr. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `impactr.json` w katalogu projektu:
...
Pola `limit` pozwalają impactr zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `impactr auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację impactr i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji impactr.

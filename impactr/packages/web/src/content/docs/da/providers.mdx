---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Impactr.
---

import config from "../../../../config.mjs"
export const console = config.console

Impactr bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Impactr-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/impactr/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Impactr-konfigurationen.
...
Impactr Zen er en liste over modeller leveret af Impactr-teamet, som er blevet testet og verificeret til at fungere godt med Impactr. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Impactr Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg impactr og gå til [impactr.dev/auth](https://impactr.dev/auth).
...
Det fungerer som alle andre udbydere i Impactr og er helt valgfrit at bruge.

---

## Impactr Go

Impactr Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Impactr-teamet, som er testet og verificeret til at fungere godt med Impactr.

1. Kør kommandoen `/connect` i TUI, vælg `Impactr Go`, og gå til [impactr.dev/auth](https://impactr.dev/zen).
...
Sådan bruger du Amazon Bedrock med Impactr:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører impactr:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `impactr.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Impactr understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere impactr til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at impactr skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem impactr-konfigurationen.
...
gemme tokenet i impactr auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Impactr bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Impactr
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`impactr.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [impactr-gitlab-auth](https://www.npmjs.com/package/impactr-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `impactr.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Impactr:
...
Sådan bruger du Google Vertex AI med Impactr:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem impactr, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [impactr-helicone-session](https://github.com/H2Shami/impactr-helicone-session) plugin for automatisk at logge hver Impactr-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Impactr-samtale opført som en separat session.
...
Du kan konfigurere impactr til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere impactr til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere impactr til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Impactr. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/impactr) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Impactr:
...
3. Kopier API-nøglen til brug i Impactr.
...
6. **Vigtigt**: Før du bruger skymodeller i Impactr, skal du hente modelinformationen lokalt:
...
Impactr Zen er en liste over testede og verificerede modeller leveret af Impactr-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Impactr Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Impactr Zen**.

   ```txt
   /connect
   ```

3. Indtast Impactr API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem impactr-konfigurationen.
...
5. Du kan også tilpasse dem gennem impactr-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Impactr:
...
Du kan også tilpasse modeller gennem impactr-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med impactr. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `impactr.json`-filen i projektkataloget:
...
`limit`-felterne lader Impactr forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `impactr auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek impactr-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i impactr-konfigurationen.

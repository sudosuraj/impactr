...
Med Impactr kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Impactr vårt eget `impactr`-tema.
...
Impactr kommer med flere innebygde temaer.
...
- Vil at Impactr skal matche terminalens utseende
...
Impactr støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/impactr/themes/*.json` eller `$XDG_CONFIG_HOME/impactr/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.impactr/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.impactr/themes/*.json`

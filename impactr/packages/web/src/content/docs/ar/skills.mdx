...
تتيح مهارات الوكيل لـ Impactr اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Impactr في هذه المواقع:

- إعدادات المشروع: `.impactr/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/impactr/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Impactr صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.impactr/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/impactr/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.impactr/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Impactr المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `impactr.json`:
...
**للوكلاء المدمجين** (في `impactr.json`):

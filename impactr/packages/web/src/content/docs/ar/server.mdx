---
title: الخادم
description: تفاعل مع خادم impactr عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `impactr serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل impactr استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `impactr.local` |
...
عيّن `IMPACTR_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `impactr`، أو عيّن `IMPACTR_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `impactr serve` و `impactr web`.
...
عند تشغيل `impactr` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم impactr للتفاعل مع impactr برمجيا.
:::

تتيح هذه البنية لـ impactr دعم عدة عملاء وتمكّنك من التفاعل مع impactr برمجيا.

يمكنك تشغيل `impactr serve` لبدء خادم مستقل. إذا كانت واجهة impactr في terminal (TUI)
قيد التشغيل، فسيبدأ `impactr serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Impactr لـ [IDE](/docs/ide).
...
يعرض خادم impactr واجهات API التالية.

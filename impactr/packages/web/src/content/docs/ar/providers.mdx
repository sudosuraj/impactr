---
title: المزوّدون
description: استخدام أي موفّر LLM في Impactr.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Impactr ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Impactr.
...
في `~/.local/share/impactr/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Impactr.
...
Impactr Zen هي قائمة نماذج يوفّرها فريق Impactr وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Impactr. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Impactr Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر impactr، ثم انتقل إلى [impactr.dev/auth](https://impactr.dev/auth).
...
يعمل مثل أي مزوّد آخر في Impactr واستخدامه اختياري بالكامل.

---

## Impactr Go

Impactr Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Impactr، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Impactr.

1. شغّل الأمر `/connect` في TUI، واختر `Impactr Go`، ثم انتقل إلى [impactr.dev/auth](https://impactr.dev/zen).
...
لاستخدام Amazon Bedrock مع Impactr:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل impactr:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `impactr.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Impactr غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين impactr لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل impactr بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات impactr.
...
تخزين الرمز في مساحة تخزين مصادقة impactr.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Impactr نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Impactr
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`impactr.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [impactr-gitlab-auth](https://www.npmjs.com/package/impactr-gitlab-auth).

##### التهيئة

خصّص عبر `impactr.json`:
...
لاستخدام اشتراك GitHub Copilot مع impactr:
...
لاستخدام Google Vertex AI مع Impactr:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر impactr، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [impactr-helicone-session](https://github.com/H2Shami/impactr-helicone-session) لتسجيل كل محادثة Impactr تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Impactr مدرجة كجلسة مستقلة.
...
يمكنك ضبط impactr لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط impactr لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط impactr لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Impactr. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/impactr) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Impactr:
...
3. انسخ مفتاح API لاستخدامه في Impactr.
...
6. **مهم**: قبل استخدام نماذج السحابة في Impactr، يجب سحب معلومات النموذج محليا:
...
Impactr Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Impactr. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Impactr Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Impactr Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Impactr.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات impactr.
...
5. يمكنك أيضا تخصيصها عبر إعدادات impactr. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Impactr:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات impactr. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع impactr. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `impactr.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Impactr بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `impactr auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات impactr وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات impactr.

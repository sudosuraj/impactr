---
title: LSP serveri
description: Impactr se integriše sa vašim LSP serverima.
---

Impactr se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

Impactr dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `IMPACTR_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i impactr otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj impactr konfiguraciji.

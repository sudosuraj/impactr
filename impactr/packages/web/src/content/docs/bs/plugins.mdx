---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Impactr.
---

Dodaci vam omogućavaju da proširite Impactr spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Impactr.
...
- `.impactr/plugins/` - Dodaci na nivou projekta
- `~/.config/impactr/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/impactr/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/impactr/impactr.json`)
2. Konfiguracija projekta (`impactr.json`)
3. Globalni direktorij dodataka (`~/.config/impactr/plugins/`)
4. Direktorij dodataka projekta (`.impactr/plugins/`)
...
Impactr pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Impactr SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Impactr.
...
Spriječite impactr da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u impactr:
...
Pomoćnik `tool` kreira prilagođeni alat koji impactr može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za impactr zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://impactr.dev/docs/sdk) za detalje.

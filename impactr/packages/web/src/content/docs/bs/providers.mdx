---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Impactr.
---

import config from "../../../../config.mjs"
export const console = config.console

Impactr koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Impactr konfiguraciji.
...
u `~/.local/share/impactr/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Impactr
config.
...
Impactr Zen je lista modela koje je obezbedio Impactr tim koji su bili
testirano i potvrđeno da dobro radi sa Impactr. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Impactr Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite impactr i idite na [impactr.dev/auth](https://impactr.dev/auth).
...
Radi kao i svaki drugi provajder u Impactr i potpuno je opcionalan za korištenje.

---

## Impactr Go

Impactr Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Impactr tim i koji su testirani i verificirani da dobro rade s Impactr-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Impactr Go` i idite na [impactr.dev/auth](https://impactr.dev/zen).
...
Da biste koristili Amazon Bedrock s Impactr:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete impactr:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `impactr.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Impactr.
:::
...
Možete konfigurirati impactr za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi impactr ispravno radio.
   :::
...
Također možete dodati modele kroz svoju impactr konfiguraciju.
...
da pohrani token u impactr auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Impactr koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Impactr
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`impactr.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [impactr-gitlab-auth](https://www.npmjs.com/package/impactr-gitlab-auth).

##### Konfiguracija

Prilagodite putem `impactr.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s impactr:
...
Za korištenje Google Vertex AI s Impactr:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće impactr.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem impactra, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [impactr-helicone-session](https://github.com/H2Shami/impactr-helicone-session) da automatski prijavite svaki Impactr razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Impactr razgovor naveden kao zasebna sesija.
...
Možete konfigurirati impactr za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati impactr za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati impactr za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Impactr. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/impactr) za detalje.
:::
...
Da biste koristili Ollama Cloud s Impactr:
...
3. Kopirajte API ključ za korištenje u Impactr.
...
6. **Važno**: Prije upotrebe modela oblaka u Impactr, morate lokalno povući informacije o modelu:
...
Impactr Zen je lista testiranih i verifikovanih modela koju je obezbedio Impactr tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Impactr Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Impactr Zen**.

```txt
   /connect
```

3. Unesite svoj Impactr API ključ.
...
Također možete dodati dodatne modele putem vaše impactr konfiguracije.
...
5. Također ih možete prilagoditi putem vaše impactr konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Impactrom:
...
Također možete prilagoditi modele kroz svoju impactr konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s impactr-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `impactr.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Impactr da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `impactr auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Impactr konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj impactr konfiguraciji.

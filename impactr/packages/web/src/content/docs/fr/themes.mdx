...
Avec Impactr, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Impactr utilise notre propre thème `impactr`.
...
Impactr est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Impactr corresponde à l'apparence de leur terminal
...
Impactr prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/impactr/themes/*.json` ou `$XDG_CONFIG_HOME/impactr/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.impactr/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.impactr/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/impactr/themes
vim ~/.config/impactr/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .impactr/themes
vim .impactr/themes/my-theme.json
```
...
{
  "$schema": "https://impactr.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",

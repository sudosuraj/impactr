...
Les compétences d'agent permettent à Impactr de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Impactr recherche ces emplacements :

- Configuration du projet : `.impactr/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/impactr/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Impactr remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.impactr/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/impactr/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.impactr/skills/git-release/SKILL.md` comme ceci :
...
Impactr répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `impactr.json` :
...
**Pour les agents intégrés** (dans `impactr.json`) :

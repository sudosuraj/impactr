---
title: Plugins
description: Écrivez vos propres plugins pour étendre Impactr.
---

Les plugins vous permettent d'étendre Impactr en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Impactr.
...
- `.impactr/plugins/` - Plugins au niveau du projet
- `~/.config/impactr/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/impactr/node_modules/`.
...
1. Configuration globale (`~/.config/impactr/impactr.json`)
2. Configuration du projet (`impactr.json`)
3. Répertoire global des plugins (`~/.config/impactr/plugins/`)
4. Répertoire des plugins du projet (`.impactr/plugins/`)
...
Impactr exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK impactr pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre impactr.
...
:::note
Si vous utilisez l'application de bureau Impactr, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez impactr de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à impactr :
...
L'assistant `tool` crée un outil personnalisé que impactr peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour impactr aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://impactr.dev/docs/sdk) pour plus de détails.

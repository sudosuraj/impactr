---
title: Serveur
description: Interagissez avec le serveur impactr via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `impactr serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client impactr peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `impactr.local` |
...
Définissez `IMPACTR_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `impactr`, ou définissez `IMPACTR_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `impactr serve` et à `impactr web`.
...
Lorsque vous exécutez `impactr`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur impactr pour interagir avec impactr par programmation.
:::

Cette architecture permet à impactr de prendre en charge plusieurs clients et vous permet d'interagir avec impactr par programme.

Vous pouvez exécuter `impactr serve` pour démarrer un serveur autonome. Si vous avez le
impactr TUI en cours d'exécution, `impactr serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Impactr [IDE](/docs/ide).
...
Le serveur impactr expose les API suivantes.

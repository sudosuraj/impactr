---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Impactr.
---

import config from "../../../../config.mjs"
export const console = config.console

Impactr verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Impactr-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Impactr anpassen
config.
...
Impactr Zen ist eine Liste von Modellen, die vom Impactr-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Impactr funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Impactr Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Impactr aus und gehen Sie zu [impactr.dev/auth](https://impactr.dev/auth).
...
Es funktioniert wie jeder andere Anbieter in Impactr und ist völlig optional.

---

## Impactr Go

Impactr Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Impactr-Team getestet und verifiziert wurden, dass sie gut mit Impactr funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Impactr Go` und gehen Sie zu [impactr.dev/auth](https://impactr.dev/zen).
...
So verwenden Sie Amazon Bedrock mit Impactr:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Impactr ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `impactr.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Impactr wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können impactr so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Impactr ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Impactr-Konfiguration hinzufügen.
...
um das Token im Impactr-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Impactr verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Impactr
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`impactr.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [impactr-gitlab-auth](https://www.npmjs.com/package/impactr-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `impactr.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Impactr:
...
So verwenden Sie Google Vertex AI mit Impactr:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Impactr ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Impactr konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [impactr-helicone-session](https://github.com/H2Shami/impactr-helicone-session)-Plugin, um jede Impactr-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Impactr-Konversation als separate Sitzung aufgeführt.
...
Sie können Impactr für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Impactr über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Impactr über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Impactr konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/impactr).
:::
...
So verwenden Sie Ollama Cloud mit Impactr:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Impactr.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Impactr verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Impactr Zen ist eine Liste getesteter und verifizierter Modelle, die vom Impactr-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Impactr Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Impactr Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Impactr API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Impactr-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Impactr-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Impactr:
...
Sie können Modelle auch über Ihre Impactr-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Impactr verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `impactr.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Impactr erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `impactr auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Impactr-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Impactr-Konfiguration übereinstimmt.

...
description: Cree herramientas que LLM pueda llamar en impactr.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de impactr como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.impactr/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/impactr/tools/`.
...
```ts title=".impactr/tools/database.ts" {1}
...
```ts title=".impactr/tools/math.ts"
import { tool } from "@impactr-ai/plugin"
...
```ts title=".impactr/tools/bash.ts"
import { tool } from "@impactr-ai/plugin"
...
```ts title=".impactr/tools/project.ts" {8}
import { tool } from "@impactr-ai/plugin"
...
```python title=".impactr/tools/add.py"
import sys
...
```ts title=".impactr/tools/python-add.ts" {10}
import { tool } from "@impactr-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".impactr/tools/add.py")

---
title: Servidor
description: Interactuar con el servidor impactr a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `impactr serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente impactr puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `impactr.local` |
...
Configure `IMPACTR_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `impactr`, o configure `IMPACTR_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `impactr serve` como a `impactr web`.
...
Cuando ejecuta `impactr`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor impactr para interactuar con impactr mediante programación.
:::

Esta arquitectura permite que impactr admita múltiples clientes y le permite interactuar con impactr mediante programación.

Puede ejecutar `impactr serve` para iniciar un servidor independiente. Si tienes el
impactr TUI ejecutándose, `impactr serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Impactr [IDE](/docs/ide).
...
El servidor impactr expone las siguientes API.

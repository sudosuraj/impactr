...
Con Impactr puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Impactr usa nuestro propio tema `impactr`.
...
Impactr viene con varios temas integrados.
...
- Quiere que Impactr coincida con la apariencia de su terminal
...
Impactr admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/impactr/themes/*.json` o `$XDG_CONFIG_HOME/impactr/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.impactr/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.impactr/themes/*.json`

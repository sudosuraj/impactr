...
brew install anomalyco/tap/impactr # macOS 與 Linux（推薦，始終保持最新）
brew install impactr              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S impactr            # Arch Linux (Stable)
paru -S impactr-bin               # Arch Linux (Latest from AUR)
mise use -g impactr               # 任何作業系統
nix run nixpkgs#impactr           # 或使用 github:anomalyco/impactr 以取得最新開發分支
```
...
1. `$IMPACTR_INSTALL_DIR` - 自定義安裝目錄
2. `$XDG_BIN_DIR` - 符合 XDG 基礎目錄規範的路徑
3. `$HOME/bin` - 標準使用者執行檔目錄 (若存在或可建立)
4. `$HOME/.impactr/bin` - 預設備用路徑
...
Impactr 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
如果您有興趣參與 Impactr 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 Impactr 進行開發

如果您正在開發與 Impactr 相關的專案，並在名稱中使用了 "impactr"（例如 "impactr-dashboard" 或 "impactr-mobile"），請在您的 README 中加入聲明，說明該專案並非由 Impactr 團隊開發，且與我們沒有任何隸屬關係。

---

**加入我們的社群** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=738j8655-cd59-4633-a30a-1124e0096789&qr_code=true) | [X.com](https://x.com/impactr)

...
brew install anomalyco/tap/impactr # macOS 및 Linux (권장, 항상 최신)
brew install impactr              # macOS 및 Linux (공식 brew formula, 업데이트 빈도 낮음)
sudo pacman -S impactr            # Arch Linux (Stable)
paru -S impactr-bin               # Arch Linux (Latest from AUR)
mise use -g impactr               # 어떤 OS든
nix run nixpkgs#impactr           # 또는 github:anomalyco/impactr 로 최신 dev 브랜치
```
...
Impactr 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
Impactr 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.

### Impactr 기반으로 만들기

Impactr 와 관련된 프로젝트를 진행하면서 이름에 "impactr"(예: "impactr-dashboard" 또는 "impactr-mobile") 를 포함한다면, README 에 해당 프로젝트가 Impactr 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.

---

**커뮤니티에 참여하기** [Discord](https://discord.gg/impactr) | [X.com](https://x.com/impactr)

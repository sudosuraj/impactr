# Remove `packages/impactr/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/impactr/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the impactr legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/impactr/src/storage/db.ts` are concentrated in 22 source files:

- `packages/impactr/src/account/repo.ts`
- `packages/impactr/src/cli/cmd/db.ts`
- `packages/impactr/src/cli/cmd/import.ts`
- `packages/impactr/src/cli/cmd/stats.ts`
- `packages/impactr/src/control-plane/workspace.ts`
- `packages/impactr/src/index.ts`
- `packages/impactr/src/node.ts`
- `packages/impactr/src/permission/index.ts`
- `packages/impactr/src/project/project.ts`
- `packages/impactr/src/server/projectors.ts`
- `packages/impactr/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/impactr/src/server/shared/fence.ts`
- `packages/impactr/src/session/message-v2.ts`
- `packages/impactr/src/session/projectors.ts`
- `packages/impactr/src/session/prompt.ts`
- `packages/impactr/src/session/session.ts`
- `packages/impactr/src/session/todo.ts`
- `packages/impactr/src/share/share-next.ts`
- `packages/impactr/src/storage/db.ts`
- `packages/impactr/src/sync/index.ts`
- `packages/impactr/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy impactr database wrapper; `packages/impactr/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the impactr wrapper.
...
Status: Completed. `SyncEvent` and the impactr projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy impactr database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy impactr database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy impactr database wrapper.
...
All migration groups are complete or superseded. `packages/impactr/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No impactr data-migration group remains.

The previous impactr `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate impactr data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/impactr`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/impactr/src`
- `bun typecheck` from `packages/impactr`
- Relevant package tests from `packages/impactr`, not the repo root

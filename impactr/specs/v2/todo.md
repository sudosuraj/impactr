...
The impactr server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@impactr-ai/llm` tool loop after replacing its
...
- plugins get global "impactr" instance like in that post i showed
- impactr instance has stuff like `impactr.session.prompt()` or
  `impactr.tool.register({...})`
...
- decide whether to preserve deprecated `@impactr-ai/llm` orchestration exports

...
Both `action` and `resource` use impactr's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.impactr` files is intentionally deferred until `.impactr` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether impactr permits an operation through its managed execution paths.

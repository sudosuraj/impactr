...
Move the canonical Impactr terminal application from
`packages/impactr/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/impactr`,
`packages/cli`, or `@impactr-ai/core`.

The SDK is the TUI's Impactr boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/impactr`.
- Do not replace private `packages/impactr` imports with `@impactr-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@impactr-ai/tui`.
...
- Impactr domain data displayed by the TUI
...
```text
packages/impactr/src/cli/cmd/tui
```

Its private dependency on `packages/impactr` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/impactr/src/*`.
...
Status: Completed. The private `@impactr-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@impactr-ai/tui`.
...
- The package has no dependency on `impactr`, `@impactr-ai/cli`, or
  `@impactr-ai/core`.
...
primitives now live in `@impactr-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@impactr-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Impactr globals or process state;
...
Replace process-global Impactr state with resolved TUI inputs.
...
`@impactr-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/impactr` initially.
...
canonical in `@impactr-ai/tui`. Configured references resolve through the new
...
adapters rather than Impactr domain implementations.

Make the SDK the only Impactr domain boundary used by the TUI.
...
- Domain-facing TUI code imports Impactr data and operations only from
  `@impactr-ai/sdk`.
...
`@impactr-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/impactr`.
...
- The package has no imports from `packages/impactr`, `packages/cli`, or
  `@impactr-ai/core`.
...
public `@impactr-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/impactr`.
- Keep the legacy embedded worker and server startup in `packages/impactr`.
- Change those adapters to load config, create transport inputs, and call the
  public `@impactr-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/impactr`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@impactr-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@impactr-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Impactr domain state.
...
cd packages/impactr && bun typecheck
...
rg '@impactr-ai/core|packages/impactr|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/impactr/src packages/impactr/test
```
...
- Build the current-platform `packages/impactr` binary.
